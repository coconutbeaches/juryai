/**
 * Shared, spec-driven external relay submission: prepare / rebase / apply.
 *
 * Extracted from `src/v2-1-4/external-relay-submission.ts`, which remains the
 * frozen behavioural reference and continues to serve every production V2.1.4
 * dispute. Nothing outside `src/tests/` imports this module.
 *
 * PARITY IS THE OBJECTIVE. Rule order, check order, reason codes, messages,
 * issue arrays and mutation ordering are preserved verbatim, because all of
 * them are observable. The only changes are:
 *
 *  - generation-scoped literals come from a ValidatedGenerationSpec;
 *  - contract-issue codes are composed from the spec's explicit prefix;
 *  - envelope validation, party-cursor refresh and explanatory-state
 *    derivation arrive as injected collaborators rather than direct imports,
 *    so the dependency direction stays `relay -> validator interface` and the
 *    engine never reaches into a frozen generation.
 *
 * The slot rules below are DELIBERATELY the frozen ones. A party that already
 * holds one live position in a `(requirement, proposition_type)` slot cannot
 * add a second, and one compile run cannot emit two assertions for the same
 * slot. That is the limitation the production canary exposed; 8C1 owns
 * changing it, and reproducing it exactly here is what makes 8C1's change
 * reviewable as a deliberate semantic decision rather than a drift.
 */
...
import {
  ...
  type CanonicalSemanticPosition,
  type CaseEnvelope,
  type FormationChallenge,
  type FormationClarification,
  type PartyId,
...
import {
  ...
  type ExternalRelayCanonicalIds,
  type TrustedExternalRelayBridge,
  type TrustedExternalRelayRuntime,
} from './relay-runtime.js';
import type { FormationEnvelopeValidator } from './validator-port.js';

export type { ExternalRelayCanonicalIds, TrustedExternalRelayBridge, TrustedExternalRelayRuntime };

/** Refreshes party-visible cursors after a mutation, returning who moved. */
export type PartyViewCursorRefresh = (before: CaseEnvelope, candidate: CaseEnvelope) => PartyId[];

export interface ExternalRelaySubmissionIntent {
...
export interface SemanticAssertionCandidateEffect {
...
export interface ClarificationRequestEffect {
...
export interface ChallengeCandidateEffect {
...
export interface ChallengeResponseCandidateEffect {
  type: 'challenge_response_candidate';
  challenge_id: string;
  statement: string;
  spans: TurnSpan[];
  semantic_correction: SemanticAssertionCandidateEffect | null;
}

export type ExternalRelayEffectCandidate =
  | SemanticAssertionCandidateEffect
  | ClarificationRequestEffect
  | ChallengeCandidateEffect
  | ChallengeResponseCandidateEffect;

export interface PreparedSemanticAssertionEffect extends SemanticAssertionCandidateEffect {
  position_id: string;
}

export interface PreparedClarificationRequestEffect extends ClarificationRequestEffect {
  clarification_id: string;
}

export interface PreparedChallengeEffect extends ChallengeCandidateEffect {
  challenge_id: string;
}

export interface PreparedChallengeResponseEffect extends Omit<
  ChallengeResponseCandidateEffect,
  'semantic_correction'
> {
  response_id: string;
  semantic_correction: PreparedSemanticAssertionEffect | null;
}

export type PreparedExternalRelayEffect =
  | PreparedSemanticAssertionEffect
  | PreparedClarificationRequestEffect
  | PreparedChallengeEffect
  | PreparedChallengeResponseEffect;

export interface ExternalRelayCompilerIdentity {
...
export interface ExternalRelaySubmission {
  ...
  compiler_run: ExternalRelayCompilerIdentity;
  effects: PreparedExternalRelayEffect[];
}
...
/**
 * Builds the relay for one generation.
 *
 * `validator` is the port, never a concrete generation's validator; `cursors`
...
export function createFormationRelay(input: {
  ...
  function candidateEffectShape(effect: unknown): effect is ExternalRelayEffectCandidate {
      ...
      case 'challenge_response_candidate':
        return (
          ...
          (typed.semantic_correction === null ||
            (typed.semantic_correction as ExternalRelayEffectCandidate).type ===
  ...
  function preparedEffectShape(effect: unknown): effect is PreparedExternalRelayEffect {
    if (typeof effect !== 'object' || effect === null || !('type' in effect)) return false;
    const typed = effect as Record<string, unknown>;
    switch (typed.type) {
      case 'semantic_assertion_candidate':
      ...
      case 'challenge_response_candidate':
        return (
          ...
          (typed.semantic_correction === null ||
            (typed.semantic_correction as PreparedExternalRelayEffect).type ===
              'semantic_assertion_candidate') &&
          typeof typed.challenge_id === 'string' &&
  ...
  function idsHaveScope(ids: ExternalRelayCanonicalIds, partyId: PartyId): boolean {
    return (
      ID_PATTERN.test(ids.submission_id) &&
      ids.submission_id.startsWith(`submission_${partyId}_`) &&
      isPartyScopedId('turn', partyId, ids.source_turn_id) &&
      ids.position_ids.every((id) => isPartyScopedId('position', partyId, id)) &&
      ids.clarification_ids.every((id) => isPartyScopedId('clarification', partyId, id)) &&
      ids.challenge_ids.every((id) => isPartyScopedId('challenge', partyId, id)) &&
      ids.challenge_response_ids.every((id) => isPartyScopedId('challenge_response', partyId, id))
    );
  }

  function requiredIdCounts(effects: readonly ExternalRelayEffectCandidate[]) {
    let positions = 0;
  ...
  function replyTargetVisibleToParty(
  ...
  function prepareExternalRelaySubmission(input: {
    envelope: CaseEnvelope;
    execution_authority: AuthenticatedPartyAuthority;
    intent: ExternalRelaySubmissionIntent;
    runtime: TrustedExternalRelayRuntime;
    compiler_run: ExternalRelayCompilerIdentity;
    effects: ExternalRelayEffectCandidate[];
  }): PrepareExternalRelaySubmissionResult {
    const {
      envelope,
    ...
    if (
      ...
      !Array.isArray(effects) ||
      !effects.every(candidateEffectShape) ||
      intent.intent_version !== spec.contracts.external_relay_submission_intent_version ||
      !Number.isSafeInteger(intent.expected_party_visible_version) ||
      intent.expected_party_visible_version < 1 ||
      !HASH_PATTERN.test(intent.expected_party_projection_hash) ||
      ...
      !intent.in_reply_to.every(
        (target) =>
          ID_PATTERN.test(target) &&
          replyTargetVisibleToParty(envelope, authority.party_id, target),
      ) ||
      ...
      !ID_PATTERN.test(compiler_run.compile_run_id) ||
    ...
    let responseIndex = 0;
    const preparedEffects: PreparedExternalRelayEffect[] = effects.map((effect) => {
      switch (effect.type) {
        case 'semantic_assertion_candidate':
          return {
    ...
    return {
      status: 'prepared',
      submission: {
        submission_version: spec.contracts.external_relay_submission_version,
  ...
  type ExternalRelaySubmissionFailureReason =
    | 'invalid_envelope'
    | 'invalid_submission'
    | 'unauthorized_actor'
    | 'case_mismatch'
  ...
  interface AppliedExternalRelaySubmission {
  ...
  type ApplyExternalRelaySubmissionResult =
    | {
        status: 'applied';
        reason_code: null;
        message: string;
        ...
        result: AppliedExternalRelaySubmission;
      }
    | {
        status: 'rejected';
        reason_code: ExternalRelaySubmissionFailureReason;
        message: string;
  ...
  function rejected(
    envelope: CaseEnvelope,
    reason: ExternalRelaySubmissionFailureReason,
    message: string,
    issues: ContractIssue[] = [],
  ): ApplyExternalRelaySubmissionResult {
    return {
      status: 'rejected',
      reason_code: reason,
      message,
  ...
  function spanCommitments(
    ...
    source: SourceTurn,
    path: string,
  ): { commitments: SourceSpanCommitment[]; issues: ContractIssue[] } {
  ...
  function assertionFailure(
    envelope: CaseEnvelope,
    partyId: PartyId,
    effect: PreparedSemanticAssertionEffect,
    source: SourceTurn,
    claimedSlots: Set<string>,
    claimedTargets: Set<string>,
    path: string,
  ): { issue: ContractIssue | null; commitments: SourceSpanCommitment[] } {
    ...
    if (
      ...
      return {
        issue: {
          code: codes.explicit_absence_source,
          path,
    ...
    if (
      ...
      return {
        issue: {
          code: codes.assertion_requirement,
          path,
    ...
    if (
      ...
      !ID_PATTERN.test(effect.compiler_assertion_id) ||
      propositionTypeDescriptor(effect.proposed_type).requires_inspected_evidence
    ) {
      return {
        issue: {
          code: codes.assertion_semantics,
          path,
    ...
    if (envelope.positions[effect.position_id]) {
      return {
        issue: { code: codes.position_id_collision, path, message: 'Position id already exists.' },
        commitments: [],
      };
    }
    ...
    if (effect.supersedes_candidate === null) {
      if (existing) {
        return {
          issue: {
            code: codes.live_position_slot_collision,
            path,
      ...
      if (
        ...
        return {
          issue: {
            code: codes.supersession_target,
            path,
  ...
  function materialOwnEdit(effect: PreparedExternalRelayEffect): boolean {
    return (
  ...
  function rebaseExternalRelaySubmission(
    submission: ExternalRelaySubmission,
    current: CaseEnvelope,
  ): ExternalRelaySubmission | null {
  ...
  function applyExternalRelaySubmission(input: {
    envelope: CaseEnvelope;
    submission: ExternalRelaySubmission;
    execution_authority: AuthenticatedPartyAuthority;
  }): ApplyExternalRelaySubmissionResult {
    ...
    if (
      submission.submission_version !== spec.contracts.external_relay_submission_version ||
      !submission.submission_id.startsWith(`submission_${partyId}_`) ||
      !ID_PATTERN.test(submission.submission_id) ||
    ...
    if (
      !isPartyScopedId('turn', partyId, submission.source_turn.turn_id) ||
    ...
    if (
      !['independent_formation', 'challenge_response'].includes(envelope.control.workflow_state)
    ) {
      return rejected(
        envelope,
        'workflow_state_forbidden',
    ...
    const newPositions: CanonicalSemanticPosition[] = [];
    const newClarifications: FormationClarification[] = [];
    const newChallenges: FormationChallenge[] = [];
    const responseEffects: Array<{
      challenge: FormationChallenge;
      effect: PreparedChallengeResponseEffect;
      commitments: SourceSpanCommitment[];
      correction: CanonicalSemanticPosition | null;
    }> = [];

    const createPosition = (
      effect: PreparedSemanticAssertionEffect,
      path: string,
    ): CanonicalSemanticPosition | ApplyExternalRelaySubmissionResult => {
    ...
    for (const [index, effect] of submission.effects.entries()) {
      ...
      switch (effect.type) {
        case 'semantic_assertion_candidate': {
          if (!isPartyScopedId('position', partyId, effect.position_id)) {
        ...
        case 'clarification_request': {
          ...
          if (
            ...
            !isPartyScopedId('clarification', partyId, effect.clarification_id) ||
        ...
        case 'challenge_candidate': {
          ...
          if (
            ...
            !target ||
            target.attributed_party_id !== otherParty(partyId) ||
            target.superseded_by !== null ||
            !submission.source_turn.in_reply_to.includes(effect.target_position_id) ||
            !isPartyScopedId('challenge', partyId, effect.challenge_id) ||
        ...
        case 'challenge_response_candidate': {
          ...
          if (
            ...
            !isPartyScopedId('challenge_response', partyId, effect.response_id) ||
            Object.values(envelope.challenges).some(
          ...
          let correction: CanonicalSemanticPosition | null = null;
    ...
    for (const challenge of newChallenges) {
      ...
      candidate.positions[challenge.target_position_id]!.last_material_envelope_version =
        nextVersion;
    ...
    const changedVisibleParties = cursors(envelope, candidate);
    ...
    return {
      status: 'applied',
      reason_code: null,
      message: `${spec.identity.display_label} external relay submission applied atomically.`,
      envelope: candidate,
      prior_envelope_version: envelope.control.envelope_version,
  ...
  function conflictTurnSummariesForParty(
    envelope: CaseEnvelope,
    partyId: PartyId,
    limit = 3,
  ): ConflictTurnSummary[] {
  ...
  return {
    /** The per-factory frozen singleton `mintRuntime` compares by identity. */
    bridge: runtimes.bridge,
    mintRuntime: runtimes.mintRuntime,
    prepareExternalRelaySubmission,
    rebaseExternalRelaySubmission,
    applyExternalRelaySubmission,
    conflictTurnSummariesForParty,
  };
}
