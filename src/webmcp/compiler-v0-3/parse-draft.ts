...
import { hasExplicitAbsenceSource } from '../core-v0-3/explicit-absence.js';
import {
...
import { isEpistemicStrength, isPropositionType } from '../core-v0-3/types.js';
...
export function parseModelDraft(input: CompilerInput, rawText: string): CompilerOutput {
  ...
  const assertions: CompiledAssertion[] = array(draft.assertions, 'model_draft.assertions').map(
    (entry, index) => {
      ...
      const spans = resolveCitations(item.citations, at + '.citations', turnId, payload);
      if (
        proposedType === 'explicit_absence' &&
        !hasExplicitAbsenceSource(payload.answer.text, spans, strength)
      ) {
        throw new SemanticCompilerOutputError(
          'explicit absence must preserve the complete answer and factual strength',
          at + '.citations',
        );
      }
      return {
