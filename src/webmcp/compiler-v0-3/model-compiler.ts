...
export const DEFAULT_COMPILER_TAXONOMY_VERSION = 'juryai-p2-v0.3.0';
...
export function resolveModelCompilerOptions(
  ...
  if (
    options.taxonomy_version !== undefined &&
    options.taxonomy_version !== DEFAULT_COMPILER_TAXONOMY_VERSION
  ) {
    throw new TypeError('The explicit-absence compiler requires its additive V0.3 taxonomy.');
  }
  return deepFreeze(
    structuredClone({
...
export function modelCompilerConfigOf(resolved: ResolvedModelCompilerOptions): ModelCompilerConfig {
  return {
    ...
    input_template_version: 'juryai-compiler-input-v0.3.0',
...
export class ModelSemanticCompiler implements SemanticCompilerPort {
  ...
  async compile(input: CompilerInput, options: CompileOptions = {}): Promise<CompilerOutput> {
    ...
    if (
      input.input_template_version !== 'juryai-compiler-input-v0.3.0' ||
      input.compiler_version_id !== this.registryEntry.compiler_version_id
    ) {
      throw new TypeError('Compiler input must bind the exact V0.3 artifact and input contract.');
    }
