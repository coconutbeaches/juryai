...
import { hasExplicitAbsenceSource } from './explicit-absence.js';
import {
  canonicalSerialize,
  isPropositionType,
  isEpistemicStrength,
...
import { verifyTurnSpan, type SourceTurnRecord, type TurnSpan } from '../core/turns.js';
import type { Proposition } from './propositions.js';
import type { RequirementDefinition } from './requirements.js';

export const COMPILER_CONTRACT_VERSION = 'juryai-webmcp-compiler-contract-v0.3.0';
...
export const COMPILER_INPUT_TEMPLATE_VERSION = 'juryai-compiler-input-v0.3.0';
...
export function validateCompilerOutputForContractVersion(
  ...
  if (compilerContractVersion !== COMPILER_CONTRACT_VERSION) {
    return [
      issue(
        'compiler_contract_version_mismatch',
        path,
        'V0.3 cannot validate or reinterpret a historical compiler contract.',
      ),
    ];
  }
  const enforceAssertionSlotCardinality = true;

  if (input.input_template_version !== COMPILER_INPUT_TEMPLATE_VERSION) {
    issues.push(
      issue(
        'compiler_input_version_mismatch',
        'compiler_input.input_template_version',
        'The additive compiler requires its exact V0.3 input contract.',
      ),
    );
  }
  ...
  for (const [index, assertion] of output.assertions.entries()) {
    ...
    if (
      assertion.proposed_type === 'explicit_absence' &&
      !hasExplicitAbsenceSource(
        input.turn.payload.answer.text,
        assertion.spans,
        assertion.epistemic_strength,
      )
    ) {
      issues.push(
        issue(
          'compiler_explicit_absence_source',
          at + '.spans',
          'Explicit absence requires the complete answer with denial, qualification and attribution, and a factual epistemic strength.',
        ),
      );
    }
    if (!knownType) {
