import type { PartyReviewStateV213 } from '../../v2-1-3/party-review-state.js';
import {
  EPISTEMIC_STRENGTHS,
  HASH_PATTERN,
  ID_PATTERN,
...
export interface ParsedFirstPartyReviewV213 {
  review_page_version: 'juryai-v2.1.3-first-party-review-page-v1.0.0';
...
export interface ParsedInvitationRedemptionV213 {
  status: 'redeemed';
  review_path: string;
}
...
export function decodeInvitationRedemptionV213(value: unknown): ParsedInvitationRedemptionV213 {
...
function validateFormationProjection(
  ...
  if (projection.projection_version !== 'juryai-party-formation-projection-v2.1.3') {
...
export function decodeFirstPartyReviewV213(value: unknown): ParsedFirstPartyReviewV213 {
  const page = exactObject(value, 'V2.1.3 review page', [
  ...
  if (page.review_page_version !== 'juryai-v2.1.3-first-party-review-page-v1.0.0') {
    throw new TypeError('V2.1.3 review page version is invalid.');
  }
  const review = exactObject(page.review, 'V2.1.3 review', [
    'dispute_id',
    'formation_epoch',
  ...
  if (
    ...
    throw new TypeError('V2.1.3 workflow phase is invalid.');
  }
  if (page.disclosure_state !== 'embargoed' && page.disclosure_state !== 'disclosed') {
    throw new TypeError('V2.1.3 disclosure state is invalid.');
  }
  if (!['open', 'acknowledged', 'unavailable'].includes(String(page.own_disclosure_review))) {
    throw new TypeError('V2.1.3 disclosure review state is invalid.');
  }
  if (review.party_id !== 'party_a' && review.party_id !== 'party_b') {
    throw new TypeError('V2.1.3 party identity is invalid.');
  }
  ...
  if (
    ...
    throw new TypeError('V2.1.3 confirmation state is invalid.');
  }
  if (review.shared_readiness !== 'not_ready' && review.shared_readiness !== 'ready_for_lock') {
    throw new TypeError('V2.1.3 shared readiness is invalid.');
  }
  if (
    review.review_state_version !== 'juryai-party-review-state-v1.1.0' ||
    review.formation_projection_version !== 'juryai-party-formation-projection-v2.1.3' ||
    review.formation_readback_version !== 'juryai-party-formation-readback-v2.1.3' ||
    projection.party_id !== review.party_id ||
    projection.case_id !== review.dispute_id ||
    readback.party_id !== review.party_id ||
    readback.readback_version !== 'juryai-party-formation-readback-v2.1.3' ||
    readback.document_hash !== review.party_readback_hash ||
    readback.party_projection_hash !== review.party_projection_hash
  ) {
    throw new TypeError('V2.1.3 frozen review binding is invalid.');
  }
  if (
    ...
    throw new TypeError('V2.1.3 review counters are invalid.');
  }
  for (const hashValue of [
    ...
    digest(hashValue, 'V2.1.3 review hash');
  }
  ...
  return {
    ...
    review: review as unknown as PartyReviewStateV213,
    workflow_phase: workflow as ParsedFirstPartyReviewV213['workflow_phase'],
    disclosure_state: page.disclosure_state,
    own_disclosure_review:
      page.own_disclosure_review as ParsedFirstPartyReviewV213['own_disclosure_review'],
    can_acknowledge_disclosure_review: boolean(
