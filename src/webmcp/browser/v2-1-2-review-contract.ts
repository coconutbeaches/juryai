import type { PartyReviewStateV212 } from '../../v2-1-2/party-review-state.js';
import {
  EPISTEMIC_STRENGTHS,
  HASH_PATTERN,
  ID_PATTERN,
...
export interface ParsedFirstPartyReviewV212 {
  review_page_version: 'juryai-v2.1.2-first-party-review-page-v1.0.0';
  review: PartyReviewStateV212;
  workflow_phase:
...
function validateFormationProjection(
  ...
  if (projection.projection_version !== 'juryai-party-formation-projection-v2.1.1') {
...
export function decodeFirstPartyReviewV212(value: unknown): ParsedFirstPartyReviewV212 {
  const page = exactObject(value, 'V2.1.2 review page', [
  ...
  if (page.review_page_version !== 'juryai-v2.1.2-first-party-review-page-v1.0.0') {
    throw new TypeError('V2.1.2 review page version is invalid.');
  }
  const review = exactObject(page.review, 'V2.1.2 review', [
    'dispute_id',
    'formation_epoch',
  ...
  if (
    ...
    throw new TypeError('V2.1.2 workflow phase is invalid.');
  }
  if (page.disclosure_state !== 'embargoed' && page.disclosure_state !== 'disclosed') {
    throw new TypeError('V2.1.2 disclosure state is invalid.');
  }
  if (!['open', 'acknowledged', 'unavailable'].includes(String(page.own_disclosure_review))) {
    throw new TypeError('V2.1.2 disclosure review state is invalid.');
  }
  if (review.party_id !== 'party_a' && review.party_id !== 'party_b') {
    throw new TypeError('V2.1.2 party identity is invalid.');
  }
  ...
  if (
    ...
    throw new TypeError('V2.1.2 confirmation state is invalid.');
  }
  if (review.shared_readiness !== 'not_ready' && review.shared_readiness !== 'ready_for_lock') {
    throw new TypeError('V2.1.2 shared readiness is invalid.');
  }
  if (
    review.review_state_version !== 'juryai-party-review-state-v1.0.0' ||
    review.formation_projection_version !== 'juryai-party-formation-projection-v2.1.1' ||
    review.formation_readback_version !== 'juryai-party-formation-readback-v2.1.1' ||
    ...
    throw new TypeError('V2.1.2 frozen review binding is invalid.');
  }
  if (
    ...
    throw new TypeError('V2.1.2 review counters are invalid.');
  }
  for (const hashValue of [
    ...
    digest(hashValue, 'V2.1.2 review hash');
  }
  ...
  return {
    ...
    review: review as unknown as PartyReviewStateV212,
    workflow_phase: workflow as ParsedFirstPartyReviewV212['workflow_phase'],
    disclosure_state: page.disclosure_state,
    own_disclosure_review:
      page.own_disclosure_review as ParsedFirstPartyReviewV212['own_disclosure_review'],
    can_acknowledge_disclosure_review: boolean(
