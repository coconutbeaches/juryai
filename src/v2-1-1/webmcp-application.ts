...
import { WEBMCP_CORE_SCHEMA_VERSION, WEBMCP_PROTOCOL_VERSION } from '../webmcp/public-contract.js';
import {
...
import {
  ...
  resolveFormationReplayObjectsV211,
...
export interface FormationRelayRepositoryV211 {
  ...
  listActiveContextsForPrincipal(subjectId: string): Promise<ActiveFormationContextV211[]>;
  resolvePartyContext(
    disputeId: string,
    subjectId: string,
...
export type RelayCanonicalIdKindV211 =
  | 'submission'
  | 'source'
  | 'turn'
  | 'run'
...
export interface RelayApplicationIdsV211 {
  next(kind: RelayCanonicalIdKindV211, partyId: PartyIdV211): string;
}

export interface RelayApplicationClockV211 {
  now(): number;
}

export interface RelayApplicationSaltsV211 {
  next(): string;
}

export function randomRelayApplicationIdsV211(): RelayApplicationIdsV211 {
  return { next: (kind, partyId) => `${kind}_${partyId}_${randomUUID()}` };
}

export const systemRelayApplicationClockV211: RelayApplicationClockV211 = {
  now: () => Date.now(),
};

export function randomRelayApplicationSaltsV211(): RelayApplicationSaltsV211 {
  return { next: () => randomBytes(32).toString('hex') };
}

export interface V211PartyCaseServiceDependencies {
  authenticated_subject_id: string;
  repository: FormationRelayRepositoryV211;
  compiler: SemanticCompilerPort;
  review_url: (disputeId: string) => string;
  ids?: RelayApplicationIdsV211;
  clock?: RelayApplicationClockV211;
  salts?: RelayApplicationSaltsV211;
  relaying_agent?: string | null;
}

export interface V211PartyCaseService {
...
/** V2.1.1 rejects authority-shaped or trusted-ID additions instead of ignoring them. */
...
function interpretation(position: PartyVisiblePositionV211): RecentInterpretationSlot {
  return {
...
function livePositions(projection: PartyScopedFormationProjectionV211): PartyVisiblePositionV211[] {
  return [
...
export function projectPartyCaseStateV211(
...
function compilerProposition(
  envelope: CaseEnvelopeV211,
  position: PartyVisiblePositionV211,
): Proposition {
...
function compilerRequirement(
  requirement: PartyScopedFormationProjectionV211['own_material']['requirements'][number],
...
type CompilePlanV211 =
  ...
  | {
      ...
      requirement_ids: string[];
      requirements: RequirementDefinition[];
      target: PartyVisiblePositionV211;
    }
  | {
      ...
      requirements: RequirementDefinition[];
      challenge_id: string;
      challenged_position: PartyVisiblePositionV211;
    };

function planCompile(
  projection: PartyScopedFormationProjectionV211,
  targets: readonly string[],
): CompilePlanV211 | null {
...
function effectsForCompilerOutput(
  plan: CompilePlanV211,
  output: CompilerOutput,
  turnId: string,
  answer: string,
): ExternalRelayEffectCandidateV211[] | null {
  ...
  })) as ExternalRelayEffectCandidateV211[];
}
...
function recordedSlots(
  ...
  return resolveFormationReplayObjectsV211(envelope, response).accepted_positions.map((position) =>
    interpretation({
...
export function createV211PartyCaseService(
  dependencies: V211PartyCaseServiceDependencies,
): V211PartyCaseService {
  const ids = dependencies.ids ?? randomRelayApplicationIdsV211();
  const clock = dependencies.clock ?? systemRelayApplicationClockV211;
  const salts = dependencies.salts ?? randomRelayApplicationSaltsV211();
  ...
  return {
    ...
    getCaseState: async (query, options) => {
      try {
        ...
        if (!isV211DisputePersistenceId(disputeId)) {
      ...
      try {
        ...
        if (command.expected_case_version !== cursor.party_visible_version) {
          return {
            ...
            recent_turns: conflictTurnSummariesForPartyV211(stored.envelope, partyId),
            likely_already_recorded: conflictTurnSummariesForPartyV211(
              stored.envelope,
              partyId,
        ...
        const projection = projectPartyFormationV211(stored.envelope, partyId);
        ...
        const runtime = trustedExternalRelayRuntimeV211(TRUSTED_EXTERNAL_RELAY_BRIDGE_V211, {
          source_channel: 'webmcp_agent_relay',
          relaying_agent: dependencies.relaying_agent ?? null,
          received_at: receivedAt,
          payload_commitment_salt: salt,
        ...
        const prepared = prepareExternalRelaySubmissionV211({
          ...
          execution_authority: partyAuthorityV211(stored.envelope, partyId, 'external_relay'),
          intent: {
            intent_version: EXTERNAL_RELAY_SUBMISSION_INTENT_VERSION_V211,
        ...
        if (committed.status === 'conflict') {
          ...
          return {
            ...
            recent_turns: conflictTurnSummariesForPartyV211(committed.current.envelope, partyId),
