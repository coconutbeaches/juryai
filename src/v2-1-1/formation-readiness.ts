...
import { currentPartyConfirmationV211 } from './party-projection.js';

export interface FormationReadinessV211 {
  readiness_version: typeof FORMATION_READINESS_VERSION_V211;
  ...
  required_current_confirmations: PartyIdV211[];
...
function authoritativeReadinessParts(envelope: CaseEnvelopeV211) {
  const openRequiredFields = Object.values(envelope.requirements)
    .filter(
      (requirement) =>
        requirement.required &&
        evaluateFormationRequirementV211(envelope, requirement).status !== 'satisfied',
    )
    .map((requirement) => requirement.requirement_id)
    .sort();
  const independentFormationPrerequisites = PARTY_IDS_V211.filter(
  ...
  const ineligibleRequiredEvidence = Object.values(envelope.evidence)
    ...
    .sort();
  const requiredCurrentConfirmations = PARTY_IDS_V211.filter(
    (partyId) => currentPartyConfirmationV211(envelope, partyId) === null,
  );
  const partyBindingBlockers = PARTY_IDS_V211.filter(
    (partyId) =>
...
export function authoritativeFormationExplanatoryStateV211(
...
export function deriveFormationReadinessV211(envelope: CaseEnvelopeV211): FormationReadinessV211 {
  const parts = authoritativeReadinessParts(envelope);
  const expected = authoritativeFormationExplanatoryStateV211(envelope);
  ...
  return cloneCanonical({
    readiness_version: FORMATION_READINESS_VERSION_V211,
