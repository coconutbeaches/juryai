/**
 * Private PostgreSQL adapter for the unchanged test-only V2.1 invitation
 * contract and the incompatible V2.1.1 ceremony/envelope contract.
 */
...
export interface PostgresFormationInvitationRepositoryOptionsV211 extends PoolConfig {
  pool?: Pool;
...
function canonicalId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !ID_PATTERN_V211.test(value)) {
...
function decodeEnvelopeRow(row: FormationDisputeRow): CaseEnvelopeV211 {
  const envelope = cloneCanonical(row.envelope as CaseEnvelopeV211);
...
class RedemptionRollbackV211 extends Error {}

export class PostgresFormationInvitationRepositoryV211 implements FormationInvitationPersistencePortV21 {
  readonly #pool: Pool;
  ...
  constructor(options: PostgresFormationInvitationRepositoryOptionsV211) {
  ...
  async redeemInvitation(
    ...
    try {
      return await this.#transaction(async (client) => {
        ...
        const command = ceremonyCommandForV211(envelope, `command_invitation_${suffix}`, {
          type: 'bind_party',
          party_slot: 'party_b',
        ...
        if (updatedDispute.rowCount !== 1) {
          throw new RedemptionRollbackV211('Authoritative envelope CAS failed.');
        }

        await client.query(`insert into ${SCHEMA}.formation_commands (record) values ($1::jsonb)`, [
          canonicalSerialize({
            dispute_id: invitation.dispute_id,
            party_id: 'party_b',
        ...
        if (consumed.rowCount !== 1) {
          throw new RedemptionRollbackV211('Invitation consumption failed.');
        }
