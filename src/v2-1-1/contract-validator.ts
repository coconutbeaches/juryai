...
import {
  BILATERAL_LOCK_RECEIPT_VERSION_V211,
...
import { hashPartyFormationProjectionV211 } from './party-projection.js';
...
function exactKeys(
  ...
  if (JSON.stringify(actual) !== JSON.stringify(wanted)) {
    issues.push(issue('v211_exact_keys', path, `Expected keys ${wanted.join(', ')}.`));
  }
}
...
function validateSpanCommitment(
  span: unknown,
  source: SourceTurnV211 | undefined,
  path: string,
): ContractIssue[] {
  const issues: ContractIssue[] = [];
  if (!object(span)) return [issue('v211_span_object', path, 'Span commitment must be an object.')];
  exactKeys(
    span,
  ...
  if (typeof span.turn_id !== 'string' || !ID_PATTERN_V211.test(span.turn_id)) {
    issues.push(issue('v211_span_turn_id', `${path}.turn_id`, 'Span turn id is invalid.'));
  }
  if (!['answer', 'context'].includes(String(span.region))) {
    issues.push(issue('v211_span_region', `${path}.region`, 'Span region is invalid.'));
  }
  if (span.encoding !== 'utf16') {
    issues.push(issue('v211_span_encoding', `${path}.encoding`, "Span encoding must be 'utf16'."));
  }
  if (!safeInteger(span.start) || !safeInteger(span.end) || span.end <= span.start) {
    issues.push(issue('v211_span_bounds', path, 'Span offsets are invalid.'));
  }
  if (typeof span.quote_hash !== 'string' || !HASH_PATTERN_V211.test(span.quote_hash)) {
    issues.push(issue('v211_span_quote_hash', `${path}.quote_hash`, 'Span quote hash is invalid.'));
  }
  if (span.region === 'answer' && span.message_index !== null) {
    issues.push(
      issue('v211_span_answer_index', `${path}.message_index`, 'Answer span index must be null.'),
    );
  }
  if (span.region === 'context' && !safeInteger(span.message_index)) {
    issues.push(
      issue(
        'v211_span_context_index',
        `${path}.message_index`,
  ...
  if (source && issues.length === 0) {
    if (span.turn_id !== source.turn_id) {
      issues.push(issue('v211_span_source_mismatch', path, 'Span names a different source turn.'));
    } else {
      const committed = span as unknown as SourceSpanCommitmentV211;
      ...
      if (length === undefined || committed.end > length) {
        issues.push(
          issue('v211_span_out_of_bounds', path, 'Span exceeds its canonical payload region.'),
        );
...
function validateSourceTurn(
  ...
  const issues: ContractIssue[] = [];
  if (!object(value)) return [issue('v211_source_object', path, 'Source turn must be an object.')];
  exactKeys(
    value,
    [
      'attributed_party_id',
  ...
  const partyId = value.attributed_party_id as PartyIdV211;
  if (!PARTY_IDS_V211.includes(partyId)) {
    issues.push(
      issue('v211_source_party', `${path}.attributed_party_id`, 'Source party is invalid.'),
    );
  }
  if (
    typeof value.turn_id !== 'string' ||
    value.turn_id !== key ||
    !PARTY_IDS_V211.includes(partyId) ||
    !isPartyScopedIdV211('turn', partyId, value.turn_id)
  ) {
    issues.push(
      issue('v211_source_id_scope', `${path}.turn_id`, 'Source turn id scope is invalid.'),
    );
  }
  if (value.dispute_id !== envelope.control.case_id) {
    issues.push(
      issue('v211_source_dispute', `${path}.dispute_id`, 'Source dispute id is invalid.'),
    );
  }
  if (
    PARTY_IDS_V211.includes(partyId) &&
  ...
  if (!validIso(value.received_at)) {
    issues.push(
      issue('v211_source_received_at', `${path}.received_at`, 'Received time is invalid.'),
    );
  }
  if (!SOURCE_CHANNELS.includes(value.source_channel as never)) {
    issues.push(
      issue('v211_source_channel', `${path}.source_channel`, 'Source channel is invalid.'),
    );
  }
  if (!stringOrNull(value.relaying_agent) || !stringOrNull(value.source_language)) {
    issues.push(issue('v211_source_metadata', path, 'Source relay/language metadata is invalid.'));
  }
  if (typeof value.translation_indicated !== 'boolean') {
    issues.push(
      issue(
        'v211_source_translation',
  ...
  if (
    ...
    !value.in_reply_to.every((entry) => ID_PATTERN_V211.test(entry))
  ) {
    issues.push(
      issue('v211_source_reply_targets', `${path}.in_reply_to`, 'Reply targets are invalid.'),
    );
  }
  if (
    ...
    issues.push(
      issue('v211_source_client_turn', `${path}.client_turn_id`, 'Client turn id is invalid.'),
    );
  }
  if (
    typeof value.request_fingerprint !== 'string' ||
    !HASH_PATTERN_V211.test(value.request_fingerprint)
  ) {
    issues.push(
      issue('v211_source_fingerprint', `${path}.request_fingerprint`, 'Fingerprint is invalid.'),
    );
  }
  if (
    ...
    !HASH_PATTERN_V211.test(value.payload_commitment)
  ) {
    issues.push(issue('v211_source_commitment', path, 'Payload commitment material is invalid.'));
  }
  if (typeof value.compile_run_id !== 'string' || !ID_PATTERN_V211.test(value.compile_run_id)) {
    issues.push(
      issue('v211_source_compile_run', `${path}.compile_run_id`, 'Compile run id is invalid.'),
    );
  }
  if (!object(value.payload_layout)) {
    issues.push(
      issue('v211_source_layout', `${path}.payload_layout`, 'Payload layout is invalid.'),
    );
  } else {
    exactKeys(
  ...
  if (value.payload !== null) {
    if (!object(value.payload)) {
      issues.push(issue('v211_source_payload', `${path}.payload`, 'Payload is invalid.'));
    } else {
      const payload = value.payload as unknown as SourceTurnV211['payload'];
      try {
        ...
        if (canonicalSerialize(normalizePayload(payload!)) !== canonicalSerialize(payload)) {
          issues.push(
            issue(
              'v211_source_payload_not_normalized',
              `${path}.payload`,
        ...
        if (
          ...
          issues.push(
            issue(
              'v211_source_payload_commitment',
        ...
        if (
          ...
          issues.push(
            issue(
              'v211_source_fingerprint_mismatch',
  ...
  if (
    ...
    issues.push(issue('v211_source_redaction', path, 'Source redaction metadata is inconsistent.'));
  }
  return issues;
}

export function validateCaseEnvelopeV211(value: unknown): ContractIssue[] {
  const issues: ContractIssue[] = [];
  try {
    canonicalSerialize(value as never);
  } catch {
    return [issue('v211_envelope_json', 'envelope', 'Envelope must be canonical JSON data.')];
  }
  if (!object(value))
    return [issue('v211_envelope_object', 'envelope', 'Envelope must be an object.')];
  exactKeys(
    value,
    [
      'challenges',
  ...
  if (!object(value.control)) {
    return [
      ...issues,
      issue('v211_control_object', 'envelope.control', 'Control must be an object.'),
    ];
  }
  exactKeys(
    value.control,
  ...
  for (const [key, expected] of versions) {
    if (value.control[key] !== expected) {
      issues.push(
        issue('v211_contract_version', `envelope.control.${key}`, `Expected ${expected}.`),
      );
    }
  }
  if (
    ...
    issues.push(issue('v211_case_id', 'envelope.control.case_id', 'Dispute id is invalid.'));
  }
  if (
    ![
    ...
    issues.push(
      issue('v211_workflow', 'envelope.control.workflow_state', 'Workflow state is invalid.'),
    );
  }
  if (!safeInteger(value.control.envelope_version, 1)) {
    issues.push(
      issue(
        'v211_envelope_version',
  ...
  if (
    ...
    !HASH_PATTERN_V211.test(value.control.envelope_hash)
  ) {
    issues.push(
      issue('v211_envelope_hash', 'envelope.control.envelope_hash', 'Envelope hash is invalid.'),
    );
  }
  if (!['embargoed', 'disclosed'].includes(String(value.control.disclosure_state))) {
    issues.push(
      issue('v211_disclosure', 'envelope.control.disclosure_state', 'Disclosure state is invalid.'),
    );
  }
  if (!object(value.control.party_views)) {
    issues.push(
      issue('v211_party_views', 'envelope.control.party_views', 'Party views are invalid.'),
    );
  } else {
    exactKeys(value.control.party_views, PARTY_IDS_V211, 'envelope.control.party_views', issues);
    for (const partyId of PARTY_IDS_V211) {
      ...
      if (
        ...
        !HASH_PATTERN_V211.test(cursor.party_projection_hash)
      ) {
        issues.push(
          issue(
            'v211_party_cursor',
  ...
  if (!object(value.parties)) {
    issues.push(issue('v211_parties', 'envelope.parties', 'Parties are invalid.'));
  } else {
    exactKeys(value.parties, PARTY_IDS_V211, 'envelope.parties', issues);
    for (const partyId of PARTY_IDS_V211) {
      const party = value.parties[partyId];
      if (!object(party)) {
        issues.push(
          issue('v211_party', `envelope.parties.${partyId}`, 'Party binding is invalid.'),
        );
        continue;
      }
      exactKeys(
      ...
      if (party.party_id !== partyId || party.role !== partyId)
        issues.push(
          issue('v211_party_role', `envelope.parties.${partyId}`, 'Party role is invalid.'),
        );
      if (
        ...
        issues.push(
          issue('v211_party_state', `envelope.parties.${partyId}`, 'Party state is invalid.'),
        );
      }
      ...
      if (
        ...
        bound !==
          (typeof party.binding_event_id === 'string' &&
            isPartyScopedIdV211('binding', partyId, party.binding_event_id))
      ) {
        issues.push(
          issue(
            'v211_party_binding',
  ...
  const envelope = value as unknown as CaseEnvelopeV211;
  for (const collection of [
    ...
    if (!object(value[collection]))
      issues.push(
        issue('v211_collection', `envelope.${collection}`, `${collection} must be an object.`),
      );
  }
  ...
  if (object(value.requirements)) {
    for (const [key, raw] of Object.entries(value.requirements)) {
      ...
      if (!object(raw)) {
        issues.push(issue('v211_requirement_object', path, 'Requirement must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      if (
        ...
        !ID_PATTERN_V211.test(key) ||
        !PARTY_IDS_V211.includes(raw.party_id as PartyIdV211) ||
        ...
        issues.push(issue('v211_requirement_shape', path, 'Requirement shape is invalid.'));
      }
      ...
      if (
        ...
        issues.push(
          issue('v211_requirement_cardinality', path, 'Requirement cardinality is invalid.'),
        );
      }
    }
  }
  ...
  if (object(value.positions)) {
    for (const [key, raw] of Object.entries(value.positions)) {
      ...
      if (!object(raw)) {
        issues.push(issue('v211_position_object', path, 'Position must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      const partyId = raw.attributed_party_id as PartyIdV211;
      if (
        raw.position_id !== key ||
        !PARTY_IDS_V211.includes(partyId) ||
        typeof raw.position_id !== 'string' ||
        !isPartyScopedIdV211('position', partyId, raw.position_id)
      )
        issues.push(
          issue('v211_position_id_scope', `${path}.position_id`, 'Position id scope is invalid.'),
        );
      if (
      ...
      if (
        ...
        issues.push(
          issue('v211_position_semantics', path, 'Position type or epistemic strength is invalid.'),
        );
      if (
        ...
        issues.push(
          issue('v211_position_statement', path, 'Position statement or resolution is invalid.'),
        );
      const source =
      ...
      if (
        ...
        !HASH_PATTERN_V211.test(raw.compiler_version_id)
      )
        issues.push(issue('v211_position_provenance', path, 'Position provenance is invalid.'));
      ...
      if (
        ...
        issues.push(issue('v211_position_history', path, 'Position history is invalid.'));
      ...
      if (
        ...
        issues.push(
          issue(
            'v211_position_supersession_shape',
            path,
    ...
    for (const position of Object.values(envelope.positions)) {
      ...
      if (position.superseded_by === null) {
        ...
        if (liveSlots.has(slot))
          issues.push(
            issue(
              'v211_live_position_slot_duplicate',
              'envelope.positions',
      ...
      if (position.superseded_by !== null) {
        ...
        if (!next || next.supersedes !== position.position_id)
          issues.push(
            issue(
              'v211_supersession_reverse_link',
              `envelope.positions.${position.position_id}`,
  ...
  if (object(value.clarifications)) {
    ...
    for (const [key, raw] of Object.entries(value.clarifications)) {
      ...
      if (!object(raw)) {
        issues.push(issue('v211_clarification_object', path, 'Clarification must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      const partyId = raw.party_id as PartyIdV211;
      if (
        raw.clarification_id !== key ||
        !PARTY_IDS_V211.includes(partyId) ||
        typeof raw.clarification_id !== 'string' ||
        !isPartyScopedIdV211('clarification', partyId, raw.clarification_id) ||
        typeof raw.requirement_id !== 'string' ||
        envelope.requirements[raw.requirement_id]?.party_id !== partyId ||
        ...
        issues.push(issue('v211_clarification_shape', path, 'Clarification shape is invalid.'));
      if (typeof raw.reopened_as === 'string') {
        ...
        if (!reopened || reopened.party_id !== partyId) {
          issues.push(
            issue(
              'v211_clarification_reopen_link',
              `${path}.reopened_as`,
      ...
      if (raw.resolved_at_envelope_version === null && typeof raw.requirement_id === 'string') {
        if (openByRequirement.has(raw.requirement_id))
          issues.push(
            issue(
              'v211_duplicate_open_clarification',
              path,
  ...
  if (object(value.evidence)) {
    for (const [key, raw] of Object.entries(value.evidence)) {
      ...
      if (!object(raw)) {
        issues.push(issue('v211_evidence_object', path, 'Evidence must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      if (
        ...
        !ID_PATTERN_V211.test(raw.evidence_id) ||
        !PARTY_IDS_V211.includes(raw.attributed_party_id as PartyIdV211) ||
        typeof raw.description !== 'string' ||
        typeof raw.required_for_readiness !== 'boolean' ||
        !['pending', 'eligible', 'ineligible', 'not_required'].includes(String(raw.eligibility))
      )
        issues.push(issue('v211_evidence_shape', path, 'Evidence shape is invalid.'));
    }
  }
  ...
  if (object(value.challenges)) {
    for (const [key, raw] of Object.entries(value.challenges)) {
      ...
      if (!object(raw)) {
        issues.push(issue('v211_challenge_object', path, 'Challenge must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      const challenger = raw.challenging_party_id as PartyIdV211;
      const targetParty = raw.target_party_id as PartyIdV211;
      const target =
      ...
      if (
        ...
        !PARTY_IDS_V211.includes(challenger) ||
        typeof raw.challenge_id !== 'string' ||
        !isPartyScopedIdV211('challenge', challenger, raw.challenge_id) ||
        targetParty !== otherPartyV211(challenger) ||
        !target ||
        ...
        issues.push(
          issue('v211_challenge_shape', path, 'Challenge shape or provenance is invalid.'),
        );
      if (!Array.isArray(raw.source_span_commitments) || raw.source_span_commitments.length === 0)
        issues.push(issue('v211_challenge_spans', path, 'Challenge needs source spans.'));
      else
        raw.source_span_commitments.forEach((span, index) =>
          issues.push(
            ...validateSpanCommitment(span, source, `${path}.source_span_commitments[${index}]`),
      ...
      if (raw.response === null) {
        if (raw.status === 'resolved')
          issues.push(
            issue('v211_challenge_response_missing', path, 'Resolved challenge needs a response.'),
          );
      } else if (!object(raw.response)) {
        issues.push(
          issue(
            'v211_challenge_response_object',
            `${path}.response`,
        ...
        if (
          ...
          !isPartyScopedIdV211('challenge_response', targetParty, raw.response.response_id) ||
          !responseSource ||
          ...
          !HASH_PATTERN_V211.test(raw.response.compiler_version_id) ||
        ...
        if (typeof raw.response.semantic_position_id === 'string') {
          ...
          if (
            ...
            issues.push(
              issue(
                'v211_challenge_response_position',
    ...
    if (
      ...
      issues.push(
        issue(
          'v211_challenge_before_disclosure',
          'envelope.challenges',
  ...
  if (!object(value.formation)) {
    issues.push(issue('v211_formation', 'envelope.formation', 'Formation state is invalid.'));
  } else {
    exactKeys(
      value.formation,
      ['confirmations', 'explanatory', 'reopen_events'],
      'envelope.formation',
      issues,
    );
    if (!object(value.formation.confirmations))
      ...
      for (const partyId of PARTY_IDS_V211) {
        ...
        for (const [index, raw] of receipts.entries()) {
          ...
          if (!object(raw)) {
            issues.push(issue('v211_confirmation_object', path, 'Confirmation must be an object.'));
            continue;
          }
          exactKeys(
            raw,
          ...
          if (
            raw.confirmation_version !== PARTY_CONFIRMATION_VERSION_V211 ||
            raw.party_id !== partyId ||
            typeof raw.confirmation_id !== 'string' ||
            !isPartyScopedIdV211('confirmation', partyId, raw.confirmation_id) ||
            typeof raw.event_id !== 'string' ||
            !isPartyScopedIdV211('confirmation_event', partyId, raw.event_id) ||
            ...
            ].every((hash) => typeof hash === 'string' && HASH_PATTERN_V211.test(hash))
          )
            issues.push(issue('v211_confirmation_shape', path, 'Confirmation receipt is invalid.'));
          if (ids.has(String(raw.confirmation_id)) || events.has(String(raw.event_id)))
            issues.push(
              issue('v211_confirmation_id_collision', path, 'Confirmation identities collide.'),
            );
    ...
    if (!Array.isArray(value.formation.reopen_events))
      ...
      for (const [index, raw] of value.formation.reopen_events.entries()) {
        ...
        if (!object(raw)) {
          issues.push(issue('v211_reopen_object', path, 'Reopen event must be an object.'));
          continue;
        }
        exactKeys(
          raw,
        ...
        if (
          !PARTY_IDS_V211.includes(partyId) ||
          typeof raw.event_id !== 'string' ||
          !isPartyScopedIdV211('reopen_event', partyId, raw.event_id) ||
          ...
          issues.push(issue('v211_reopen_shape', path, 'Reopen event is invalid.'));
  ...
  if (issues.length === 0) {
    for (const partyId of PARTY_IDS_V211) {
      if (
        ...
        hashPartyFormationProjectionV211(envelope, partyId)
      )
        issues.push(
          issue(
            'v211_party_projection_hash',
            `envelope.control.party_views.${partyId}.party_projection_hash`,
            'Stored party projection hash is stale.',
          ),
        );
    }
    const expectedExplanatory = authoritativeFormationExplanatoryStateV211(envelope);
    if (
      canonicalSerialize(envelope.formation.explanatory) !== canonicalSerialize(expectedExplanatory)
    )
      issues.push(
        issue(
          'v211_explanatory_mismatch',
          'envelope.formation.explanatory',
          'Stored explanatory state is stale.',
        ),
      );
    if (envelope.control.envelope_hash !== hashCaseEnvelopeV211(envelope))
      issues.push(
        issue(
          'v211_envelope_hash_mismatch',
...
export function assertValidCaseEnvelopeV211(envelope: CaseEnvelopeV211): void {
  const issues = validateCaseEnvelopeV211(envelope);
  if (issues.length > 0) throw new TypeError(`${issues[0]!.code}: ${issues[0]!.message}`);
}

void BILATERAL_LOCK_RECEIPT_VERSION_V211;
