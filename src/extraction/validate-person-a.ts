...
function emptyRegistry(): IdRegistry {
  return {
    party: new Set(), third_party: new Set(), submission: new Set(), agreement_term: new Set(),
    deliverable: new Set(), timeline: new Set(), claim: new Set(), evidence: new Set(),
    evidence_extract: new Set(), claim_evidence_link: new Set(), damages: new Set(),
    outcome: new Set(), extraction_issue: new Set(), clarification_question: new Set(),
  };
}
...
function collectIds(record: JsonObject, issues: ValidationIssue[]): IdRegistry {
  ...
  array(record.deliverable_assessments).forEach((item, index) =>
    register('deliverable', item.deliverable_id, `$.deliverable_assessments[${index}].deliverable_id`),
  );
  ...
  array(record.evidence).forEach((item, index) => {
    ...
    array(item.extracts).forEach((extract, extractIndex) =>
      register('evidence_extract', extract.extract_id, `$.evidence[${index}].extracts[${extractIndex}].extract_id`),
    );
  });
  ...
  array(record.clarification_questions).forEach((item, index) =>
    register('clarification_question', item.question_id, `$.clarification_questions[${index}].question_id`),
  );
  return registry;
}
...
function validateReferences(record: JsonObject, ids: IdRegistry, issues: ValidationIssue[]): void {
  ...
  array(record.deliverable_assessments).forEach((item, index) => {
    ...
    each(item.source_evidence_ids, 'evidence', `$.deliverable_assessments[${index}].source_evidence_ids`);
  });
  array(record.timeline).forEach((item, index) => {
    if (item.actor_party_id) expect(item.actor_party_id, 'party', `$.timeline[${index}].actor_party_id`);
    if (item.actor_third_party_id) expect(item.actor_third_party_id, 'third_party', `$.timeline[${index}].actor_third_party_id`);
  ...
  array(record.claims).forEach((item, index) => {
    ...
    each(item.contradicting_evidence_ids, 'evidence', `$.claims[${index}].contradicting_evidence_ids`);
  ...
  const linkable: IdFamily[] = [
    'agreement_term', 'deliverable', 'timeline', 'claim', 'evidence', 'damages', 'outcome', 'extraction_issue',
  ];
  array(record.extraction_issues).forEach((item, index) =>
  ...
  array(record.clarification_questions).forEach((item, index) => {
    ...
    each(item.answer_evidence_ids, 'evidence', `$.clarification_questions[${index}].answer_evidence_ids`);
  });
}

function tracedObjects(record: JsonObject): Array<{ spans: unknown; path: string }> {
  return [
    ...array(record.agreement?.terms).map((item, index) => ({ spans: item.source_spans, path: `$.agreement.terms[${index}].source_spans` })),
    ...array(record.timeline).map((item, index) => ({ spans: item.source_spans, path: `$.timeline[${index}].source_spans` })),
    ...array(record.claims).map((item, index) => ({ spans: item.source_spans, path: `$.claims[${index}].source_spans` })),
    ...array(record.extraction_issues).map((item, index) => ({ spans: item.source_spans, path: `$.extraction_issues[${index}].source_spans` })),
  ];
}
...
export function validatePersonAExtraction(record: unknown, narrative: string): PersonAValidationResult {
  ...
  if (record.submission.content_hash !== expectedHash)
    add(invariantErrors, '$.submission.content_hash', 'The submission content_hash must equal sha256(narrative).');
  if (record.metadata.input_hash !== expectedHash)
    add(invariantErrors, '$.metadata.input_hash', 'The metadata input_hash must equal sha256(narrative).');

  for (const traced of tracedObjects(record)) {
    const spans = array(traced.spans);
    if (spans.length === 0) add(invariantErrors, traced.path, 'Narrative-derived objects require at least one source span.');
    spans.forEach((span, index) => {
      ...
      if (narrative.slice(span.start_char, span.end_char) !== span.quote)
        add(invariantErrors, path, `Source span does not match narrative.slice(${span.start_char}, ${span.end_char}).`);
    });
  }
  ...
  array(record.claims).forEach((claim, index) => {
    ...
    if (!['none', 'not_assessed'].includes(claim.support_level)) add(invariantErrors, `$.claims[${index}].support_level`, 'Uninspected evidence cannot receive evidentiary support weight.');
  });
  ...
  array(record.evidence).forEach((evidence, index) => {
    ...
    if (!['described_only', 'unavailable'].includes(evidence.availability_status)) add(invariantErrors, `${path}.availability_status`, 'Narrative extraction may only create described_only or unavailable evidence.');
    ...
    if (evidence.original_filename !== null && !narrative.includes(evidence.original_filename))
      add(invariantErrors, `${path}.original_filename`, 'original_filename must be null unless the exact filename appears in the narrative.');
  ...
  array(record.claim_evidence_links).forEach((link, index) => {
    if (link.strength !== 'not_assessed')
      add(invariantErrors, `$.claim_evidence_links[${index}].strength`, 'Uninspected evidence links must remain not_assessed.');
  });
  ...
  if (record.desired_outcomes.party_id !== 'party_a') add(invariantErrors, '$.desired_outcomes.party_id', 'Desired outcomes must belong to party_a.');
