...
import { alignPersonA } from '../alignment/person-a-alignment.js';
...
function parseArgs(argv: string[]): Args {
  ...
  return {
    input: resolve(
      projectRoot,
      values.get('input') ?? 'src/fixtures/dry_run_001.person_a.txt',
    ),
    submittedAt: values.get('submitted-at') ?? '2026-07-18T12:00:00Z',
    model: values.get('model') ?? process.env.JURYAI_MODEL ?? 'gpt-5.1',
...
async function main(): Promise<void> {
  ...
  if (args.extraction) {
    ...
    const result = await extractPersonA({
      ...
      model: args.model,
      client,
      reasoningEffort: (process.env.JURYAI_REASONING_EFFORT as
        | 'low'
        | 'medium'
        | 'high'
        | 'xhigh'
