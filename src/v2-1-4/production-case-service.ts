...
import type {
  ...
  CommitControlledDisclosureInputV214,
...
export interface ProductionFormationRepositoryV214 extends FormationRelayRepositoryV214 {
  createDispute(
...
export function createInitialProductionDisputeV214(input: {
  ...
  const bound = applyEnvelopeCeremonyCommandV214({
    envelope,
    command: ceremonyCommandForV214(envelope, `command_start_${digest.slice(0, 32)}`, {
      type: 'bind_party',
      party_slot: 'party_a',
...
function disclosureCommandId(stored: StoredFormationDisputeV214): string {
  return `command_disclosure_${stored.internal_envelope_hash.slice(0, 32)}`;
}

export async function attemptControlledDisclosureV214(
  repository: ProductionFormationRepositoryV214,
  stored: StoredFormationDisputeV214,
): Promise<StoredFormationDisputeV214> {
  if (
...
export interface ProductionCaseServiceV214 extends CaseServicePort {
  listActiveCaseIds(options?: ServiceCallOptions): Promise<string[]>;
}

export function createProductionCaseServiceV214(input: {
  authenticated_subject_id: string;
  repository: ProductionFormationRepositoryV214;
  compiler: SemanticCompilerPort;
  review_url: (disputeId: string) => string;
  idempotency_secret: string;
}): ProductionCaseServiceV214 {
  const partyService: V214PartyCaseService = createV214PartyCaseService(input);
  const refreshSuccessfulSubmit = async (
    ...
    await attemptControlledDisclosureV214(input.repository, stored);
  ...
  return {
    ...
    startCase: async (command: StartCaseCommand, options): Promise<StartCaseResult> => {
      try {
        ...
        const envelope = createInitialProductionDisputeV214({
          authenticated_subject_id: input.authenticated_subject_id,
