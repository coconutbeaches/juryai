...
import {
  conflictTurnSummariesForPartyV214,
...
import {
  isV214DisputePersistenceId,
  resolveFormationReplayObjectsV214,
...
import {
  ...
  type PartyScopedFormationProjectionV214,
  type PartyVisiblePositionV214,
...
export interface FormationRelayRepositoryV214 {
  findById(disputeId: string): Promise<StoredFormationDisputeV214 | null>;
  listActiveContextsForPrincipal(subjectId: string): Promise<ActiveFormationContextV214[]>;
  resolvePartyContext(
    disputeId: string,
    subjectId: string,
...
export type RelayCanonicalIdKindV214 =
  | 'submission'
  | 'source'
  | 'turn'
  | 'run'
...
export interface RelayApplicationIdsV214 {
  next(kind: RelayCanonicalIdKindV214, partyId: PartyIdV214): string;
}

export interface RelayApplicationClockV214 {
  now(): number;
}

export interface RelayApplicationSaltsV214 {
  next(): string;
}

export function randomRelayApplicationIdsV214(): RelayApplicationIdsV214 {
  return { next: (kind, partyId) => `${kind}_${partyId}_${randomUUID()}` };
}

export const systemRelayApplicationClockV214: RelayApplicationClockV214 = {
  now: () => Date.now(),
};

export function randomRelayApplicationSaltsV214(): RelayApplicationSaltsV214 {
  return { next: () => randomBytes(32).toString('hex') };
}

export interface V214PartyCaseServiceDependencies {
  authenticated_subject_id: string;
  repository: FormationRelayRepositoryV214;
  compiler: SemanticCompilerPort;
  review_url: (disputeId: string) => string;
  ids?: RelayApplicationIdsV214;
  clock?: RelayApplicationClockV214;
  salts?: RelayApplicationSaltsV214;
  relaying_agent?: string | null;
}

export interface V214PartyCaseService {
...
/** V2.1.4 rejects authority-shaped or trusted-ID additions instead of ignoring them. */
...
function interpretation(position: PartyVisiblePositionV214): RecentInterpretationSlot {
  return {
...
function livePositions(projection: PartyScopedFormationProjectionV214): PartyVisiblePositionV214[] {
  return [
...
export function projectPartyCaseStateV214(
...
function compilerProposition(
  envelope: CaseEnvelopeV214,
  position: PartyVisiblePositionV214,
): Proposition {
...
function compilerRequirement(
  requirement: PartyScopedFormationProjectionV214['own_material']['requirements'][number],
...
type CompilePlanV214 =
  ...
  | {
      ...
      requirement_ids: string[];
      requirements: RequirementDefinition[];
      target: PartyVisiblePositionV214;
    }
  | {
      ...
      requirements: RequirementDefinition[];
      challenge_id: string;
      challenged_position: PartyVisiblePositionV214;
    };

function planCompile(
  projection: PartyScopedFormationProjectionV214,
  targets: readonly string[],
): CompilePlanV214 | null {
...
function effectsForCompilerOutput(
  plan: CompilePlanV214,
  output: CompilerOutput,
  turnId: string,
  answer: string,
): ExternalRelayEffectCandidateV214[] | null {
...
function recordedSlots(
  ...
  return resolveFormationReplayObjectsV214(envelope, response).accepted_positions.map((position) =>
    interpretation({
...
export function createV214PartyCaseService(
  dependencies: V214PartyCaseServiceDependencies,
): V214PartyCaseService {
  if (
    ...
    throw new TypeError('V2.1.4 requires the V0.3 compiler and taxonomy contracts.');
  }
  const ids = dependencies.ids ?? randomRelayApplicationIdsV214();
  const clock = dependencies.clock ?? systemRelayApplicationClockV214;
  const salts = dependencies.salts ?? randomRelayApplicationSaltsV214();
  ...
  return {
    ...
    getCaseState: async (query, options) => {
      try {
        ...
        if (!isV214DisputePersistenceId(disputeId)) {
    ...
    submitTurn: async (command, options) => {
      try {
        options?.signal?.throwIfAborted();
        if (!isV214DisputePersistenceId(command.case_id)) {
          return serviceError('CASE_NOT_FOUND', 'No such case.');
        }
        ...
        if (command.expected_case_version !== cursor.party_visible_version) {
          return {
            ...
            recent_turns: conflictTurnSummariesForPartyV214(stored.envelope, partyId),
            likely_already_recorded: conflictTurnSummariesForPartyV214(
              stored.envelope,
              partyId,
        ...
        const projection = projectPartyFormationV214(stored.envelope, partyId);
        ...
        const runtime = trustedExternalRelayRuntimeV214(TRUSTED_EXTERNAL_RELAY_BRIDGE_V214, {
          source_channel: 'webmcp_agent_relay',
          relaying_agent: dependencies.relaying_agent ?? null,
          received_at: receivedAt,
          payload_commitment_salt: salt,
        ...
        const prepared = prepareExternalRelaySubmissionV214({
          ...
          execution_authority: partyAuthorityV214(stored.envelope, partyId, 'external_relay'),
          intent: {
            intent_version: EXTERNAL_RELAY_SUBMISSION_INTENT_VERSION_V214,
        ...
        if (committed.status === 'conflict') {
          ...
          return {
            ...
            recent_turns: conflictTurnSummariesForPartyV214(committed.current.envelope, partyId),
