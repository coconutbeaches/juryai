...
import { deriveFormationReadinessV214 } from './formation-readiness.js';
import {
  currentPartyConfirmationV214,
  projectPartyFormationV214,
  renderPartyFormationReadbackV214,
  type PartyFormationReadbackV214,
  type PartyScopedFormationProjectionV214,
} from './party-projection.js';

export const PARTY_REVIEW_STATE_VERSION_V214 = 'juryai-party-review-state-v1.2.0';

export interface PartyReviewStateV214 {
  review_state_version: typeof PARTY_REVIEW_STATE_VERSION_V214;
  dispute_id: string;
  party_id: PartyIdV214;
  formation_projection_version: typeof PARTY_FORMATION_PROJECTION_VERSION_V214;
  party_projection_hash: string;
  party_visible_version: number;
  formation_readback_version: typeof PARTY_FORMATION_READBACK_VERSION_V214;
  party_readback_hash: string;
  formation_epoch: number;
  formation_projection: PartyScopedFormationProjectionV214;
  formation_readback: PartyFormationReadbackV214;
...
function reviewHashProjection(value: Omit<PartyReviewStateV214, 'review_state_hash'>): JsonValue {
  return cloneCanonical(value) as unknown as JsonValue;
}

export function hashPartyReviewStateV214(
  value: Omit<PartyReviewStateV214, 'review_state_hash'>,
): string {
  return sha256(canonicalSerialize(reviewHashProjection(value)));
}

export function derivePartyReviewStateV214(
  envelope: CaseEnvelopeV214,
  partyId: PartyIdV214,
): PartyReviewStateV214 {
  const formationProjection = projectPartyFormationV214(envelope, partyId);
  const formationReadback = renderPartyFormationReadbackV214(envelope, partyId);
  const cursor = envelope.control.party_views[partyId];
  const reviewWithoutHash: Omit<PartyReviewStateV214, 'review_state_hash'> = {
    review_state_version: PARTY_REVIEW_STATE_VERSION_V214,
    dispute_id: envelope.control.case_id,
    party_id: partyId,
    formation_projection_version: PARTY_FORMATION_PROJECTION_VERSION_V214,
    party_projection_hash: cursor.party_projection_hash,
    party_visible_version: cursor.party_visible_version,
    formation_readback_version: PARTY_FORMATION_READBACK_VERSION_V214,
    ...
    own_confirmation_state:
      currentPartyConfirmationV214(envelope, partyId) === null ? 'unconfirmed' : 'confirmed',
    shared_readiness: deriveFormationReadinessV214(envelope).ready_for_bilateral_lock
      ? 'ready_for_lock'
      : 'not_ready',
  };
  return cloneCanonical({
    ...reviewWithoutHash,
    review_state_hash: hashPartyReviewStateV214(reviewWithoutHash),
  });
}
...
export function validatePartyReviewStateV214(value: unknown): ContractIssue[] {
  ...
  if (record.review_state_version !== PARTY_REVIEW_STATE_VERSION_V214) {
    issues.push(
      issue(
  ...
  if (
    record.formation_projection_version !== PARTY_FORMATION_PROJECTION_VERSION_V214 ||
    record.formation_readback_version !== PARTY_FORMATION_READBACK_VERSION_V214
  ) {
    issues.push(
      issue(
        'party_review_state_frozen_contract',
        '$.formation_projection_version',
        'Party review state must reference the frozen V2.1.4 formation contracts.',
      ),
    );
  }
  if (
...
export function derivePartyConfirmationEligibilityV214(
  envelope: CaseEnvelopeV214,
  partyId: PartyIdV214,
): PartyConfirmationEligibilityV1 {
  const readiness = deriveFormationReadinessV214(envelope);
  ...
  if (currentPartyConfirmationV214(envelope, partyId) !== null) {
