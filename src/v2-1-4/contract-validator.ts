...
import { hashPartyFormationProjectionV214 } from './party-projection.js';
...
function exactKeys(
  ...
  if (JSON.stringify(actual) !== JSON.stringify(wanted)) {
    issues.push(issue('v214_exact_keys', path, `Expected keys ${wanted.join(', ')}.`));
  }
}
...
function validateSpanCommitment(
  span: unknown,
  source: SourceTurnV214 | undefined,
  path: string,
): ContractIssue[] {
  const issues: ContractIssue[] = [];
  if (!object(span)) return [issue('v214_span_object', path, 'Span commitment must be an object.')];
  exactKeys(
    span,
  ...
  if (typeof span.turn_id !== 'string' || !ID_PATTERN_V214.test(span.turn_id)) {
    issues.push(issue('v214_span_turn_id', `${path}.turn_id`, 'Span turn id is invalid.'));
  }
  if (!['answer', 'context'].includes(String(span.region))) {
    issues.push(issue('v214_span_region', `${path}.region`, 'Span region is invalid.'));
  }
  if (span.encoding !== 'utf16') {
    issues.push(issue('v214_span_encoding', `${path}.encoding`, "Span encoding must be 'utf16'."));
  }
  if (!safeInteger(span.start) || !safeInteger(span.end) || span.end <= span.start) {
    issues.push(issue('v214_span_bounds', path, 'Span offsets are invalid.'));
  }
  if (typeof span.quote_hash !== 'string' || !HASH_PATTERN_V214.test(span.quote_hash)) {
    issues.push(issue('v214_span_quote_hash', `${path}.quote_hash`, 'Span quote hash is invalid.'));
  }
  if (span.region === 'answer' && span.message_index !== null) {
    issues.push(
      issue('v214_span_answer_index', `${path}.message_index`, 'Answer span index must be null.'),
    );
  }
  if (span.region === 'context' && !safeInteger(span.message_index)) {
    issues.push(
      issue(
        'v214_span_context_index',
        `${path}.message_index`,
  ...
  if (source && issues.length === 0) {
    if (span.turn_id !== source.turn_id) {
      issues.push(issue('v214_span_source_mismatch', path, 'Span names a different source turn.'));
    } else {
      const committed = span as unknown as SourceSpanCommitmentV214;
      ...
      if (length === undefined || committed.end > length) {
        issues.push(
          issue('v214_span_out_of_bounds', path, 'Span exceeds its canonical payload region.'),
        );
...
function validateSourceTurn(
  ...
  const issues: ContractIssue[] = [];
  if (!object(value)) return [issue('v214_source_object', path, 'Source turn must be an object.')];
  exactKeys(
    value,
    [
      'attributed_party_id',
  ...
  const partyId = value.attributed_party_id as PartyIdV214;
  if (!PARTY_IDS_V214.includes(partyId)) {
    issues.push(
      issue('v214_source_party', `${path}.attributed_party_id`, 'Source party is invalid.'),
    );
  }
  if (
    typeof value.turn_id !== 'string' ||
    value.turn_id !== key ||
    !PARTY_IDS_V214.includes(partyId) ||
    !isPartyScopedIdV214('turn', partyId, value.turn_id)
  ) {
    issues.push(
      issue('v214_source_id_scope', `${path}.turn_id`, 'Source turn id scope is invalid.'),
    );
  }
  if (value.dispute_id !== envelope.control.case_id) {
    issues.push(
      issue('v214_source_dispute', `${path}.dispute_id`, 'Source dispute id is invalid.'),
    );
  }
  if (
    PARTY_IDS_V214.includes(partyId) &&
  ...
  if (!validIso(value.received_at)) {
    issues.push(
      issue('v214_source_received_at', `${path}.received_at`, 'Received time is invalid.'),
    );
  }
  if (!SOURCE_CHANNELS.includes(value.source_channel as never)) {
    issues.push(
      issue('v214_source_channel', `${path}.source_channel`, 'Source channel is invalid.'),
    );
  }
  if (!stringOrNull(value.relaying_agent) || !stringOrNull(value.source_language)) {
    issues.push(issue('v214_source_metadata', path, 'Source relay/language metadata is invalid.'));
  }
  if (typeof value.translation_indicated !== 'boolean') {
    issues.push(
      issue(
        'v214_source_translation',
  ...
  if (
    ...
    !value.in_reply_to.every((entry) => ID_PATTERN_V214.test(entry))
  ) {
    issues.push(
      issue('v214_source_reply_targets', `${path}.in_reply_to`, 'Reply targets are invalid.'),
    );
  }
  if (
    ...
    issues.push(
      issue('v214_source_client_turn', `${path}.client_turn_id`, 'Client turn id is invalid.'),
    );
  }
  if (
    typeof value.request_fingerprint !== 'string' ||
    !HASH_PATTERN_V214.test(value.request_fingerprint)
  ) {
    issues.push(
      issue('v214_source_fingerprint', `${path}.request_fingerprint`, 'Fingerprint is invalid.'),
    );
  }
  if (
    ...
    !HASH_PATTERN_V214.test(value.payload_commitment)
  ) {
    issues.push(issue('v214_source_commitment', path, 'Payload commitment material is invalid.'));
  }
  if (typeof value.compile_run_id !== 'string' || !ID_PATTERN_V214.test(value.compile_run_id)) {
    issues.push(
      issue('v214_source_compile_run', `${path}.compile_run_id`, 'Compile run id is invalid.'),
    );
  }
  if (!object(value.payload_layout)) {
    issues.push(
      issue('v214_source_layout', `${path}.payload_layout`, 'Payload layout is invalid.'),
    );
  } else {
    exactKeys(
  ...
  if (value.payload !== null) {
    if (!object(value.payload)) {
      issues.push(issue('v214_source_payload', `${path}.payload`, 'Payload is invalid.'));
    } else {
      const payload = value.payload as unknown as SourceTurnV214['payload'];
      try {
        ...
        if (canonicalSerialize(normalizePayload(payload!)) !== canonicalSerialize(payload)) {
          issues.push(
            issue(
              'v214_source_payload_not_normalized',
              `${path}.payload`,
        ...
        if (
          ...
          issues.push(
            issue(
              'v214_source_payload_commitment',
  ...
  if (
    ...
    issues.push(issue('v214_source_redaction', path, 'Source redaction metadata is inconsistent.'));
  }
  return issues;
}

export function validateCaseEnvelopeV214(value: unknown): ContractIssue[] {
  const issues: ContractIssue[] = [];
  try {
    canonicalSerialize(value as never);
  } catch {
    return [issue('v214_envelope_json', 'envelope', 'Envelope must be canonical JSON data.')];
  }
  if (!object(value))
    return [issue('v214_envelope_object', 'envelope', 'Envelope must be an object.')];
  exactKeys(
    value,
    [
      'challenges',
  ...
  if (!object(value.control)) {
    return [
      ...issues,
      issue('v214_control_object', 'envelope.control', 'Control must be an object.'),
    ];
  }
  exactKeys(
    value.control,
  ...
  for (const [key, expected] of versions) {
    if (value.control[key] !== expected) {
      issues.push(
        issue('v214_contract_version', `envelope.control.${key}`, `Expected ${expected}.`),
      );
    }
  }
  if (
    ...
    issues.push(issue('v214_case_id', 'envelope.control.case_id', 'Dispute id is invalid.'));
  }
  if (
    ![
    ...
    issues.push(
      issue('v214_workflow', 'envelope.control.workflow_state', 'Workflow state is invalid.'),
    );
  }
  if (!safeInteger(value.control.envelope_version, 1)) {
    issues.push(
      issue(
        'v214_envelope_version',
  ...
  if (
    ...
    !HASH_PATTERN_V214.test(value.control.envelope_hash)
  ) {
    issues.push(
      issue('v214_envelope_hash', 'envelope.control.envelope_hash', 'Envelope hash is invalid.'),
    );
  }
  if (!['embargoed', 'disclosed'].includes(String(value.control.disclosure_state))) {
    issues.push(
      issue('v214_disclosure', 'envelope.control.disclosure_state', 'Disclosure state is invalid.'),
    );
  }
  if (!object(value.control.party_views)) {
    issues.push(
      issue('v214_party_views', 'envelope.control.party_views', 'Party views are invalid.'),
    );
  } else {
    exactKeys(value.control.party_views, PARTY_IDS_V214, 'envelope.control.party_views', issues);
    for (const partyId of PARTY_IDS_V214) {
      ...
      if (
        ...
        !HASH_PATTERN_V214.test(cursor.party_projection_hash)
      ) {
        issues.push(
          issue(
            'v214_party_cursor',
  ...
  if (!object(value.parties)) {
    issues.push(issue('v214_parties', 'envelope.parties', 'Parties are invalid.'));
  } else {
    exactKeys(value.parties, PARTY_IDS_V214, 'envelope.parties', issues);
    for (const partyId of PARTY_IDS_V214) {
      const party = value.parties[partyId];
      if (!object(party)) {
        issues.push(
          issue('v214_party', `envelope.parties.${partyId}`, 'Party binding is invalid.'),
        );
        continue;
      }
      exactKeys(
      ...
      if (party.party_id !== partyId || party.role !== partyId)
        issues.push(
          issue('v214_party_role', `envelope.parties.${partyId}`, 'Party role is invalid.'),
        );
      if (
        ...
        issues.push(
          issue('v214_party_state', `envelope.parties.${partyId}`, 'Party state is invalid.'),
        );
      }
      ...
      if (
        ...
        bound !==
          (typeof party.binding_event_id === 'string' &&
            isPartyScopedIdV214('binding', partyId, party.binding_event_id))
      ) {
        issues.push(
          issue(
            'v214_party_binding',
  ...
  const envelope = value as unknown as CaseEnvelopeV214;
  for (const collection of [
    ...
    if (!object(value[collection]))
      issues.push(
        issue('v214_collection', `envelope.${collection}`, `${collection} must be an object.`),
      );
  }
  ...
  if (object(value.requirements)) {
    for (const [key, raw] of Object.entries(value.requirements)) {
      ...
      if (!object(raw)) {
        issues.push(issue('v214_requirement_object', path, 'Requirement must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      if (
        ...
        !ID_PATTERN_V214.test(key) ||
        !PARTY_IDS_V214.includes(raw.party_id as PartyIdV214) ||
        ...
        issues.push(issue('v214_requirement_shape', path, 'Requirement shape is invalid.'));
      }
      ...
      if (
        ...
        issues.push(
          issue('v214_requirement_cardinality', path, 'Requirement cardinality is invalid.'),
        );
      }
    }
  }
  ...
  if (object(value.positions)) {
    for (const [key, raw] of Object.entries(value.positions)) {
      ...
      if (!object(raw)) {
        issues.push(issue('v214_position_object', path, 'Position must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      const partyId = raw.attributed_party_id as PartyIdV214;
      if (
        raw.position_id !== key ||
        !PARTY_IDS_V214.includes(partyId) ||
        typeof raw.position_id !== 'string' ||
        !isPartyScopedIdV214('position', partyId, raw.position_id)
      )
        issues.push(
          issue('v214_position_id_scope', `${path}.position_id`, 'Position id scope is invalid.'),
        );
      if (
      ...
      if (
        ...
        issues.push(
          issue('v214_position_semantics', path, 'Position type or epistemic strength is invalid.'),
        );
      if (
        ...
        issues.push(
          issue('v214_position_statement', path, 'Position statement or resolution is invalid.'),
        );
      const source =
      ...
      if (
        ...
        !HASH_PATTERN_V214.test(raw.compiler_version_id)
      )
        issues.push(issue('v214_position_provenance', path, 'Position provenance is invalid.'));
      ...
      if (
        ...
        issues.push(issue('v214_position_history', path, 'Position history is invalid.'));
      ...
      if (
        ...
        issues.push(
          issue(
            'v214_position_supersession_shape',
            path,
    ...
    for (const position of Object.values(envelope.positions)) {
      ...
      if (position.superseded_by === null) {
        ...
        if (liveSlots.has(slot))
          issues.push(
            issue(
              'v214_live_position_slot_duplicate',
              'envelope.positions',
      ...
      if (position.superseded_by !== null) {
        ...
        if (!next || next.supersedes !== position.position_id)
          issues.push(
            issue(
              'v214_supersession_reverse_link',
              `envelope.positions.${position.position_id}`,
  ...
  if (object(value.clarifications)) {
    ...
    for (const [key, raw] of Object.entries(value.clarifications)) {
      ...
      if (!object(raw)) {
        issues.push(issue('v214_clarification_object', path, 'Clarification must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      const partyId = raw.party_id as PartyIdV214;
      if (
        raw.clarification_id !== key ||
        !PARTY_IDS_V214.includes(partyId) ||
        typeof raw.clarification_id !== 'string' ||
        !isPartyScopedIdV214('clarification', partyId, raw.clarification_id) ||
        typeof raw.requirement_id !== 'string' ||
        envelope.requirements[raw.requirement_id]?.party_id !== partyId ||
        ...
        issues.push(issue('v214_clarification_shape', path, 'Clarification shape is invalid.'));
      if (typeof raw.reopened_as === 'string') {
        ...
        if (!reopened || reopened.party_id !== partyId) {
          issues.push(
            issue(
              'v214_clarification_reopen_link',
              `${path}.reopened_as`,
  ...
  if (object(value.evidence)) {
    for (const [key, raw] of Object.entries(value.evidence)) {
      ...
      if (!object(raw)) {
        issues.push(issue('v214_evidence_object', path, 'Evidence must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      if (
        ...
        !ID_PATTERN_V214.test(raw.evidence_id) ||
        !PARTY_IDS_V214.includes(raw.attributed_party_id as PartyIdV214) ||
        typeof raw.description !== 'string' ||
        typeof raw.required_for_readiness !== 'boolean' ||
        !['pending', 'eligible', 'ineligible', 'not_required'].includes(String(raw.eligibility))
      )
        issues.push(issue('v214_evidence_shape', path, 'Evidence shape is invalid.'));
    }
  }
  ...
  if (object(value.challenges)) {
    for (const [key, raw] of Object.entries(value.challenges)) {
      ...
      if (!object(raw)) {
        issues.push(issue('v214_challenge_object', path, 'Challenge must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      const challenger = raw.challenging_party_id as PartyIdV214;
      const targetParty = raw.target_party_id as PartyIdV214;
      const target =
      ...
      if (
        ...
        !PARTY_IDS_V214.includes(challenger) ||
        typeof raw.challenge_id !== 'string' ||
        !isPartyScopedIdV214('challenge', challenger, raw.challenge_id) ||
        targetParty !== otherPartyV214(challenger) ||
        !target ||
        ...
        issues.push(
          issue('v214_challenge_shape', path, 'Challenge shape or provenance is invalid.'),
        );
      if (!Array.isArray(raw.source_span_commitments) || raw.source_span_commitments.length === 0)
        issues.push(issue('v214_challenge_spans', path, 'Challenge needs source spans.'));
      else
        raw.source_span_commitments.forEach((span, index) =>
          issues.push(
            ...validateSpanCommitment(span, source, `${path}.source_span_commitments[${index}]`),
      ...
      if (raw.response === null) {
        if (raw.status === 'resolved')
          issues.push(
            issue('v214_challenge_response_missing', path, 'Resolved challenge needs a response.'),
          );
        ...
        if (
          ...
          !isPartyScopedIdV214('challenge_response', targetParty, raw.response.response_id) ||
          !responseSource ||
          ...
          !HASH_PATTERN_V214.test(raw.response.compiler_version_id) ||
        ...
        if (typeof raw.response.semantic_position_id === 'string') {
          ...
          if (
            ...
            issues.push(
              issue(
                'v214_challenge_response_position',
  ...
  if (!object(value.formation)) {
    issues.push(issue('v214_formation', 'envelope.formation', 'Formation state is invalid.'));
  } else {
    exactKeys(
      value.formation,
    ...
    if (!object(value.formation.confirmations))
      ...
      for (const partyId of PARTY_IDS_V214) {
        ...
        for (const [index, raw] of receipts.entries()) {
          ...
          if (!object(raw)) {
            issues.push(issue('v214_confirmation_object', path, 'Confirmation must be an object.'));
            continue;
          }
          exactKeys(
            raw,
          ...
          if (
            raw.confirmation_version !== PARTY_CONFIRMATION_VERSION_V214 ||
            raw.party_id !== partyId ||
            typeof raw.confirmation_id !== 'string' ||
            !isPartyScopedIdV214('confirmation', partyId, raw.confirmation_id) ||
            typeof raw.event_id !== 'string' ||
            !isPartyScopedIdV214('confirmation_event', partyId, raw.event_id) ||
            raw.party_projection_version !== PARTY_FORMATION_PROJECTION_VERSION_V214 ||
            raw.party_readback_version !== PARTY_FORMATION_READBACK_VERSION_V214 ||
            ...
            ].every((hash) => typeof hash === 'string' && HASH_PATTERN_V214.test(hash))
          )
            issues.push(issue('v214_confirmation_shape', path, 'Confirmation receipt is invalid.'));
          if (ids.has(String(raw.confirmation_id)) || events.has(String(raw.event_id)))
            issues.push(
              issue('v214_confirmation_id_collision', path, 'Confirmation identities collide.'),
            );
    ...
    if (!Array.isArray(value.formation.reopen_events))
      ...
      for (const [index, raw] of value.formation.reopen_events.entries()) {
        ...
        if (!object(raw)) {
          issues.push(issue('v214_reopen_object', path, 'Reopen event must be an object.'));
          continue;
        }
        exactKeys(
          raw,
        ...
        if (
          !PARTY_IDS_V214.includes(partyId) ||
          typeof raw.event_id !== 'string' ||
          !isPartyScopedIdV214('reopen_event', partyId, raw.event_id) ||
          ...
          issues.push(issue('v214_reopen_shape', path, 'Reopen event is invalid.'));
  ...
  if (issues.length === 0) {
    for (const partyId of PARTY_IDS_V214) {
      if (
        ...
        hashPartyFormationProjectionV214(envelope, partyId)
      )
        issues.push(
          issue(
            'v214_party_projection_hash',
            `envelope.control.party_views.${partyId}.party_projection_hash`,
            'Stored party projection hash is stale.',
          ),
        );
    }
    const expectedExplanatory = authoritativeFormationExplanatoryStateV214(envelope);
    if (
      canonicalSerialize(envelope.formation.explanatory) !== canonicalSerialize(expectedExplanatory)
    )
      issues.push(
        issue(
          'v214_explanatory_mismatch',
          'envelope.formation.explanatory',
          'Stored explanatory state is stale.',
        ),
      );
    if (envelope.control.envelope_hash !== hashCaseEnvelopeV214(envelope))
      issues.push(
        issue(
          'v214_envelope_hash_mismatch',
...
export function assertValidCaseEnvelopeV214(envelope: CaseEnvelopeV214): void {
  const issues = validateCaseEnvelopeV214(envelope);
  if (issues.length > 0) throw new TypeError(`${issues[0]!.code}: ${issues[0]!.message}`);
}

function validateAcknowledgmentHistory(envelope: CaseEnvelopeV214, issues: ContractIssue[]): void {
  ...
  for (const partyId of PARTY_IDS_V214) {
    ...
    for (const [index, raw] of entries.entries()) {
      ...
      if (!object(raw)) {
        issues.push(issue('v214_disclosure_ack_object', path, 'Acknowledgment must be an object.'));
        continue;
      }
      exactKeys(
        raw,
      ...
      const shapeValid =
        raw.acknowledgment_version === DISCLOSURE_REVIEW_ACKNOWLEDGMENT_VERSION_V214 &&
        raw.dispute_id === envelope.control.case_id &&
        raw.party_id === partyId &&
        typeof raw.acknowledgment_id === 'string' &&
        isPartyScopedIdV214('disclosure_ack', partyId, raw.acknowledgment_id) &&
        typeof raw.event_id === 'string' &&
        isPartyScopedIdV214('disclosure_ack_event', partyId, raw.event_id) &&
        raw.authenticated_subject_id === envelope.parties[partyId].authenticated_subject_id &&
        safeInteger(raw.formation_epoch, 1) &&
        raw.formation_epoch <= envelope.parties[partyId].formation_epoch &&
        raw.party_projection_version === PARTY_FORMATION_PROJECTION_VERSION_V214 &&
        typeof raw.party_projection_hash === 'string' &&
        HASH_PATTERN_V214.test(raw.party_projection_hash) &&
        safeInteger(raw.party_visible_version, 1) &&
        ...
        HASH_PATTERN_V214.test(raw.party_readback_hash) &&
      ...
      const currentCommitBindingValid =
        envelopeVersion !== envelope.control.envelope_version ||
        currentDisclosureReviewAcknowledgmentV214(envelope, partyId)?.acknowledgment_id ===
