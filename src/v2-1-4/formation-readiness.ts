...
import {
  ...
  type FormationExplanatoryStateV214,
  type PartyIdV214,
...
import { currentPartyConfirmationV214 } from './party-projection.js';

export interface FormationReadinessV214 {
  readiness_version: typeof FORMATION_READINESS_VERSION_V214;
  ...
  required_current_confirmations: PartyIdV214[];
...
function authoritativeReadinessParts(envelope: CaseEnvelopeV214) {
  const openRequiredFields = Object.values(envelope.requirements)
    .filter(
      (requirement) =>
        requirement.required &&
        evaluateFormationRequirementV214(envelope, requirement).status !== 'satisfied',
    )
    .map((requirement) => requirement.requirement_id)
    .sort();
  const independentFormationPrerequisites = PARTY_IDS_V214.filter(
  ...
  const disclosureReviewPrerequisites = PARTY_IDS_V214.filter(
    (partyId) => currentDisclosureReviewAcknowledgmentV214(envelope, partyId) === null,
  ...
  const ineligibleRequiredEvidence = Object.values(envelope.evidence)
    ...
    .sort();
  const requiredCurrentConfirmations = PARTY_IDS_V214.filter(
    (partyId) => currentPartyConfirmationV214(envelope, partyId) === null,
  );
  const partyBindingBlockers = PARTY_IDS_V214.filter(
    (partyId) =>
...
export function authoritativeFormationExplanatoryStateV214(
  envelope: CaseEnvelopeV214,
): FormationExplanatoryStateV214 {
...
export function deriveFormationReadinessV214(envelope: CaseEnvelopeV214): FormationReadinessV214 {
  const parts = authoritativeReadinessParts(envelope);
  const expected = authoritativeFormationExplanatoryStateV214(envelope);
  ...
  return cloneCanonical({
    readiness_version: FORMATION_READINESS_VERSION_V214,
