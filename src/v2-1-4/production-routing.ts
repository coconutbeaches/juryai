...
import type { ProductionCaseServiceV214 } from './production-case-service.js';
import type { ProductionFirstPartyServiceV214 } from './production-first-party.js';
import {
...
/**
 * Every generation that can still be READ in production. V2.1.4 is the current
 * writer; V2.1.2 and V2.1.3 stay readable and writable exactly as persisted,
 * because a dispute's semantics are fixed when it is created and never migrate.
 */
export type ProductionFormationVersion =
  'juryai-case-envelope-v2.1.2' | 'juryai-case-envelope-v2.1.3' | 'juryai-case-envelope-v2.1.4';
...
export function createProductionVersionedCaseServiceV214(input: {
  enabled: boolean;
  ...
  if (!input.enabled || !input.v214 || !input.v213 || !input.v212) return input.legacy;
  const { v212, v213, v214 } = input;
  ...
  return {
    startCase: async (command, options) => {
      ...
      // of a start that already created a V2.1.2 or V2.1.3 dispute resolves to
      // THAT dispute in its own generation rather than creating a V2.1.4 twin.
      ...
      return existing
        ...
        : v214.startCase(command, options);
    },
    getCaseState: async (query, options) => {
      ...
      const [legacy, v212Ids, v213Ids, v214Ids] = await Promise.all([
      ...
      const ids = [...v212Ids, ...v213Ids, ...v214Ids];
