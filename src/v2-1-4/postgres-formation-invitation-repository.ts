...
export function productionInvitationAuthorityV214(
  enabled: boolean,
): TrustedProductionInvitationAuthorityV214 | null {
  return enabled ? TRUSTED_PRODUCTION_INVITATION_AUTHORITY_V214 : null;
}

function authorized(value: unknown): value is TrustedProductionInvitationAuthorityV214 {
  return value === TRUSTED_PRODUCTION_INVITATION_AUTHORITY_V214;
}
...
export interface PostgresFormationInvitationRepositoryOptionsV214 extends PoolConfig {
  pool?: Pool;
...
export type RedeemFormationInvitationResultV214 =
...
function decodeEnvelopeRow(row: FormationDisputeRow): CaseEnvelopeV214 {
...
export class PostgresFormationInvitationRepositoryV214 {
  readonly #pool: Pool;
  ...
  constructor(options: PostgresFormationInvitationRepositoryOptionsV214) {
  ...
  async issueInvitation(input: {
    authority: TrustedProductionInvitationAuthorityV214 | null;
    dispute_id: string;
  ...
  async redeemInvitation(input: {
  ...
  }): Promise<RedeemFormationInvitationResultV214> {
    if (!authorized(input.authority)) return invitationUnavailableResultV21();
    ...
    try {
      return await this.#transaction(async (client) => {
        ...
        const command = ceremonyCommandForV214(envelope, `command_invitation_${suffix}`, {
          type: 'bind_party',
          party_slot: 'party_b',
