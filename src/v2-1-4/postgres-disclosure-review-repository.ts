/** Private PostgreSQL persistence for V2.1.4 formation and review commands. */
...
import {
  type FormationCompilerRunAuditRecordV214,
  type FormationReplayRecordV214,
  type FormationReplayResponseV214,
  type FormationSourceAuditRecordV214,
  type FormationSubmissionAuditRecordV214,
...
export interface PostgresDisclosureReviewRepositoryOptionsV214 extends PoolConfig {
  pool?: Pool;
  clock?: { now: () => string };
  ids?: {
    next: (kind: PartyReviewIdentityKindV214, partyId: PartyIdV214) => string;
    public_reference: () => string;
  };
}

export type PartyReviewIdentityKindV214 =
  | 'challenge'
  | 'command'
  | 'confirmation'
  | 'confirmation_event'
...
function canonicalId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !ID_PATTERN_V214.test(value)) {
    throw new TypeError(`${label} is not canonical.`);
  }
  return value;
}

function partyId(value: unknown): PartyIdV214 {
...
function hash(value: unknown, label: string): string {
  if (typeof value !== 'string' || !HASH_PATTERN_V214.test(value)) {
...
function decodeActionPayload(value: unknown): PartyReviewProtectedActionPayloadV214 {
  const payload = cloneCanonical(value as PartyReviewProtectedActionPayloadV214);
  if (!validatePartyReviewProtectedActionPayloadV214(payload)) {
    throw new TypeError('Stored protected action payload failed its canonical contract.');
  }
  return payload;
}

function defaultReviewIdentity(kind: PartyReviewIdentityKindV214, party: PartyIdV214): string {
  const prefix: Record<PartyReviewIdentityKindV214, string> = {
...
function reviewActionIds(
  options: NonNullable<PostgresDisclosureReviewRepositoryOptionsV214['ids']>,
...
function sameCreationIdentity(requested: CaseEnvelopeV214, stored: CaseEnvelopeV214): boolean {
  const bindingIdentity = (envelope: CaseEnvelopeV214, party: PartyIdV214) => {
...
function decodeReplay(value: unknown): FormationReplayRecordV214 {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new TypeError('Replay record is invalid.');
  }
  const record = cloneCanonical(value as FormationReplayRecordV214);
  ...
  const idArray = (candidate: unknown): candidate is string[] =>
    ...
    candidate.every((id) => typeof id === 'string' && ID_PATTERN_V214.test(id));
  if (
    ...
    !HASH_PATTERN_V214.test(record.request_fingerprint) ||
    !Number.isSafeInteger(record.recorded_at_ms) ||
    Object.keys(responseRecord).sort().join(',') !== responseKeys.join(',') ||
    record.response.persistence_contract_version !== FORMATION_PERSISTENCE_CONTRACT_VERSION_V214 ||
    record.response.dispute_id !== record.dispute_id ||
    record.response.party_id !== record.party_id ||
    !ID_PATTERN_V214.test(record.response.submission_id) ||
    !ID_PATTERN_V214.test(record.response.source_turn_id) ||
    ...
    !HASH_PATTERN_V214.test(record.response.resulting_internal_envelope_hash) ||
    !Number.isSafeInteger(record.response.resulting_party_visible_version) ||
    record.response.resulting_party_visible_version < 1 ||
    !HASH_PATTERN_V214.test(record.response.resulting_party_projection_hash)
  ) {
    throw new TypeError('Replay record is invalid.');
  }
...
function decodeStored(row: StoredFormationRow): StoredFormationDisputeV214 {
  const envelope = cloneCanonical(row.envelope as CaseEnvelopeV214);
  ...
  if (
    ...
    throw new TypeError('Stored envelope identity disagrees with canonical V2.1.4 state.');
  }
  return {
    envelope,
...
function partyForSubject(envelope: CaseEnvelopeV214, subject: string): PartyIdV214 | null {
...
export class PostgresDisclosureReviewRepositoryV214 {
  readonly #pool: Pool;
  readonly #ownsPool: boolean;
  readonly #issuedContexts = new WeakSet<object>();
  readonly #clock: { now: () => string };
  readonly #ids: NonNullable<PostgresDisclosureReviewRepositoryOptionsV214['ids']>;

  constructor(options: PostgresDisclosureReviewRepositoryOptionsV214) {
    ...
    this.#ids =
      ...
      } satisfies NonNullable<PostgresDisclosureReviewRepositoryOptionsV214['ids']>);
    if (options.pool) {
  ...
  async getPartyReview(input: {
  ...
  }): ReturnType<PartyReviewPersistencePortV214['getPartyReview']> {
    ...
    const result = await this.#pool.query<{ envelope: unknown }>(
      `select envelope from ${SCHEMA}.formation_disputes
        where dispute_id = $1 and schema_version = 'juryai-case-envelope-v2.1.4'
    ...
    return party ? derivePartyReviewStateV214(envelope, party) : null;
  }

  async issuePartyReviewChallenge(
    input: Parameters<PartyReviewPersistencePortV214['issuePartyReviewChallenge']>[0],
    ...
    return this.#transaction(async (client) => {
      const selected = await client.query<{ envelope: unknown }>(
        `select envelope from ${SCHEMA}.formation_disputes
          where dispute_id = $1 and schema_version = 'juryai-case-envelope-v2.1.4' for update`,
        [id],
      );
  ...
  async executePartyReviewAction(
    input: Parameters<PartyReviewPersistencePortV214['executePartyReviewAction']>[0],
    ...
    return this.#transaction(async (client) => {
      ...
      const executed = executePartyReviewProtectedActionV214({
  ...
  async findById(idInput: string): Promise<StoredFormationDisputeV214 | null> {
  ...
  async listActiveContextsForPrincipal(
    subjectInput: string,
  ): Promise<ActiveFormationContextV214[]> {
    ...
    const result = await this.#pool.query(
      `select ${selectedFormationColumns('dispute')},
        ...
        where dispute.schema_version = 'juryai-case-envelope-v2.1.4'
  ...
  async resolvePartyContext(
    ...
    const result = await this.#pool.query(
      `select ${selectedFormationColumns('dispute')},
        ...
        where dispute.dispute_id = $1
          and dispute.schema_version = 'juryai-case-envelope-v2.1.4'
    ...
    const cursor = stored.envelope.control.party_views[party];
    const context: FormationPartyPersistenceContextV214 = Object.freeze({
      dispute_id: id,
      party_id: party,
      authenticated_subject_id: subject,
      internal_envelope_version: stored.internal_envelope_version,
  ...
  async createDispute(
    envelopeInput: CaseEnvelopeV214,
  ): Promise<{ created: boolean; stored: StoredFormationDisputeV214 }> {
    ...
    return this.#transaction(async (client) => {
      ...
      if (!sameCreationIdentity(envelope, stored.envelope)) {
        throw new TypeError('A different V2.1.4 creation identity already uses this dispute id.');
      }
  ...
  async commitExternalRelaySubmission(
    ...
    if (
      ...
      throw new TypeError('Compiler artifact does not reproduce the committed V2.1.4 run.');
    }
    ...
    return this.#transaction(async (client) => {
      ...
      if (
        ...
        const rebased = rebaseExternalRelaySubmissionV214(submission, current.envelope);
      ...
      const applied = applyExternalRelaySubmissionV214({
        ...
        submission,
        execution_authority: partyAuthorityV214(current.envelope, party, 'external_relay'),
      });
      ...
      const sourceRecord: FormationSourceAuditRecordV214 = {
      ...
      const submissionRecord: FormationSubmissionAuditRecordV214 = {
        dispute_id: current.envelope.control.case_id,
        party_id: party,
        submission_id: submission.submission_id,
      ...
      const compilerRecord: FormationCompilerRunAuditRecordV214 = {
        persistence_contract_version: FORMATION_PERSISTENCE_CONTRACT_VERSION_V214,
      ...
      const replayRecord: FormationReplayRecordV214 = {
        dispute_id: current.envelope.control.case_id,
        party_id: party,
        client_turn_id: submission.source_turn.client_turn_id,
  ...
  async commitControlledDisclosure(
    ...
    return this.#transaction(async (client) => {
      ...
      const applied = applyEnvelopeCeremonyCommandV214({
        envelope: current.envelope,
        command: ceremonyCommandForV214(current.envelope, input.command_id, {
  ...
  async commitDisclosureReviewAcknowledgment(
    ...
    return this.#transaction(async (client) => {
      ...
      const command = ceremonyCommandForV214(current.envelope, input.command_id, {
      ...
      const applied = applyEnvelopeCeremonyCommandV214({
        envelope: current.envelope,
        command,
        execution_authority: partyAuthorityV214(current.envelope, partyId, 'first_party_human'),
      });
      if (applied.status === 'rejected') {
        return {
          status: 'domain_rejected',
