...
import {
  EXTERNAL_RELAY_SUBMISSION_INTENT_VERSION_V215,
  PARTY_FORMATION_PROJECTION_VERSION_V215,
  partyAuthorityV215,
  type CanonicalSemanticPositionV215,
  type CaseEnvelopeV215,
  type PartyIdV215,
} from './case-envelope.js';
import {
  conflictTurnSummariesForPartyV215,
  prepareExternalRelaySubmissionV215,
  trustedExternalRelayRuntimeV215,
  TRUSTED_EXTERNAL_RELAY_BRIDGE_V215,
  type ExternalRelayEffectCandidateV215,
} from './external-relay-submission.js';
import {
  isV215DisputePersistenceId,
  resolveFormationReplayObjectsV215,
...
import {
  projectPartyFormationV215,
  type PartyScopedFormationProjectionV215,
  type PartyVisiblePositionV215,
...
export interface FormationRelayRepositoryV215 {
  findById(disputeId: string): Promise<StoredFormationDisputeV215 | null>;
  listActiveContextsForPrincipal(subjectId: string): Promise<ActiveFormationContextV215[]>;
  resolvePartyContext(
    disputeId: string,
    subjectId: string,
...
export type RelayCanonicalIdKindV215 =
  | 'submission'
  | 'source'
  | 'turn'
  | 'run'
...
export interface RelayApplicationIdsV215 {
  next(kind: RelayCanonicalIdKindV215, partyId: PartyIdV215): string;
}

export interface RelayApplicationClockV215 {
  now(): number;
}

export interface RelayApplicationSaltsV215 {
  next(): string;
}

export function randomRelayApplicationIdsV215(): RelayApplicationIdsV215 {
  return { next: (kind, partyId) => `${kind}_${partyId}_${randomUUID()}` };
}

export const systemRelayApplicationClockV215: RelayApplicationClockV215 = {
  now: () => Date.now(),
};

export function randomRelayApplicationSaltsV215(): RelayApplicationSaltsV215 {
  return { next: () => randomBytes(32).toString('hex') };
}

export interface V215PartyCaseServiceDependencies {
  authenticated_subject_id: string;
  repository: FormationRelayRepositoryV215;
  compiler: SemanticCompilerPort;
  review_url: (disputeId: string) => string;
  ids?: RelayApplicationIdsV215;
  clock?: RelayApplicationClockV215;
  salts?: RelayApplicationSaltsV215;
  relaying_agent?: string | null;
}

export interface V215PartyCaseService {
...
/** V2.1.5 rejects authority-shaped or trusted-ID additions instead of ignoring them. */
...
function interpretation(position: PartyVisiblePositionV215): RecentInterpretationSlot {
  return {
...
function livePositions(projection: PartyScopedFormationProjectionV215): PartyVisiblePositionV215[] {
  return [
...
export function projectPartyCaseStateV215(
...
function compilerProposition(
  envelope: CaseEnvelopeV215,
  position: PartyVisiblePositionV215,
): Proposition {
...
function compilerRequirement(
  requirement: PartyScopedFormationProjectionV215['own_material']['requirements'][number],
...
type CompilePlanV215 =
  ...
  | {
      ...
      requirement_ids: string[];
      requirements: RequirementDefinition[];
      target: PartyVisiblePositionV215;
    }
  | {
      ...
      requirements: RequirementDefinition[];
      challenge_id: string;
      challenged_position: PartyVisiblePositionV215;
    };

function planCompile(
  projection: PartyScopedFormationProjectionV215,
  targets: readonly string[],
): CompilePlanV215 | null {
  ...
  return {
    ...
    // Pacing stays in requirement_ids. Parsing sees the complete own context.
    requirements: [...ownRequirements.values()].map(compilerRequirement),
  };
}
...
function recordedSlots(
  ...
  return resolveFormationReplayObjectsV215(envelope, response).accepted_positions.map((position) =>
    interpretation({
...
export function createV215PartyCaseService(
  dependencies: V215PartyCaseServiceDependencies,
): V215PartyCaseService {
  assertQualifiedCompiler(dependencies.compiler.registryEntry);
  if (
    dependencies.compiler.registryEntry.version.schema_version !== COMPILER_CONTRACT_VERSION ||
    dependencies.compiler.registryEntry.version.taxonomy_version !==
      V215_SPEC.compiler.taxonomy_version
  ) {
    throw new TypeError('V2.1.5 requires the V0.4 compiler and V0.3 taxonomy contracts.');
  }
  const ids = dependencies.ids ?? randomRelayApplicationIdsV215();
  const clock = dependencies.clock ?? systemRelayApplicationClockV215;
  const salts = dependencies.salts ?? randomRelayApplicationSaltsV215();
  ...
  return {
    ...
    getCaseState: async (query, options) => {
      try {
        ...
        if (!isV215DisputePersistenceId(disputeId)) {
    ...
    submitTurn: async (command, options) => {
      try {
        options?.signal?.throwIfAborted();
        if (!isV215DisputePersistenceId(command.case_id)) {
          return serviceError('CASE_NOT_FOUND', 'No such case.');
        }
        ...
        if (command.expected_case_version !== cursor.party_visible_version) {
          return {
            ...
            recent_turns: conflictTurnSummariesForPartyV215(stored.envelope, partyId),
            likely_already_recorded: conflictTurnSummariesForPartyV215(
              stored.envelope,
              partyId,
        ...
        assertQualifiedCompiler(registryEntry);
        ...
        const runtime = trustedExternalRelayRuntimeV215(TRUSTED_EXTERNAL_RELAY_BRIDGE_V215, {
          source_channel: 'webmcp_agent_relay',
          relaying_agent: dependencies.relaying_agent ?? null,
          received_at: receivedAt,
          payload_commitment_salt: salt,
        ...
        const prepared = prepareExternalRelaySubmissionV215({
          envelope: stored.envelope,
          execution_authority: partyAuthorityV215(stored.envelope, partyId, 'external_relay'),
          intent: {
            intent_version: EXTERNAL_RELAY_SUBMISSION_INTENT_VERSION_V215,
          ...
          compiler_run: {
            compile_run_id: compileRunId,
            compiler_version_id: registryEntry.compiler_version_id,
            party_projection_contract_version: PARTY_FORMATION_PROJECTION_VERSION_V215,
        ...
        if (committed.status === 'conflict') {
          ...
          return {
            ...
            recent_turns: conflictTurnSummariesForPartyV215(committed.current.envelope, partyId),
