...
import {
  DISCLOSURE_REVIEW_ACKNOWLEDGMENT_STATEMENT_V215,
...
import type {
  ...
  CommitDisclosureReviewAcknowledgmentInputV215,
  CommitFinalConfirmationInputV215,
  StoredFormationDisputeV215,
} from './formation-persistence.js';
import {
  createPartyReviewApplicationV215,
  type PartyReviewApplicationV215,
  type PartyReviewPersistencePortV215,
} from './party-review-application.js';
import type { PartyReviewStateV215 } from './party-review-state.js';
import {
...
import type {
  ...
  TrustedProductionInvitationAuthorityV215,
} from './postgres-formation-invitation-repository.js';

export const PRODUCTION_PARTY_REVIEW_POLICY_PROFILE_ID_V215 =
  'juryai-v2.1.2-production-party-review-hhc3';
export const FIRST_PARTY_REVIEW_PAGE_VERSION_V215 = V215_SPEC.contracts.review_page_version;

const PRODUCTION_PARTY_REVIEW_POLICY_V215: IntentAssuranceProtocolProfileV1 = {
  policy_version: INTENT_ASSURANCE_POLICY_VERSION_V1,
  profile_id: PRODUCTION_PARTY_REVIEW_POLICY_PROFILE_ID_V215,
...
export interface ProductionFirstPartyServiceV215 {
  ...
  }): ReturnType<PostgresFormationInvitationRepositoryV215['issueInvitation']>;
  redeemInvitation(input: {
    opaque_token: string;
    authenticated_email: string;
  }): ReturnType<PostgresFormationInvitationRepositoryV215['redeemInvitation']>;
  getReview(disputeId: string): Promise<PartyReviewStateV215 | null>;
  getReviewPage(disputeId: string): Promise<ProductionFirstPartyReviewPageV215 | null>;
  acknowledgeDisclosureReview(disputeId: string): Promise<CommitCeremonyResultV215>;
...
export interface ProductionFirstPartyReviewPageV215 {
  review_page_version: typeof FIRST_PARTY_REVIEW_PAGE_VERSION_V215;
  review: PartyReviewStateV215;
  workflow_phase: CaseEnvelopeV215['control']['workflow_state'];
  disclosure_state: CaseEnvelopeV215['control']['disclosure_state'];
...
function resolvedPolicy(action: PartyReviewProtectedActionV1) {
  const decision = resolveIntentAssurancePolicyDecisionV1(
    action,
    PRODUCTION_PARTY_REVIEW_POLICY_V215,
    TRUSTED_INTENT_ASSURANCE_POLICY_RESOLVER_V1,
  );
...
function unavailable(): CommitCeremonyResultV215 {
  return { status: 'unauthorized' };
}

export function createProductionFirstPartyServiceV215(input: {
  enabled: boolean;
  authenticated_subject_id: string;
  repository: ProductionFirstPartyRepositoryV215;
  invitations: PostgresFormationInvitationRepositoryV215;
  invitation_authority: TrustedProductionInvitationAuthorityV215 | null;
  clock?: { now: () => string };
}): ProductionFirstPartyServiceV215 {
  const now = input.clock?.now ?? (() => new Date().toISOString());
  const review: PartyReviewApplicationV215 = createPartyReviewApplicationV215({
  ...
  return {
    ...
    getReviewPage: async (disputeId) => {
      ...
      const reviewState = derivePartyReviewStateV215(stored.envelope, party);
      const currentAcknowledgment = currentDisclosureReviewAcknowledgmentV215(
        stored.envelope,
        party,
      );
      ...
      const disclosureReviewAvailable =
        ...
        !openChallenge;
      return {
        review_page_version: FIRST_PARTY_REVIEW_PAGE_VERSION_V215,
        review: reviewState,
        ...
        disclosure_review_acknowledgment_statement: DISCLOSURE_REVIEW_ACKNOWLEDGMENT_STATEMENT_V215,
      };
    },
...
export function disclosureReviewAcknowledgmentCopyV215(): string {
  return DISCLOSURE_REVIEW_ACKNOWLEDGMENT_STATEMENT_V215;
}

export function reviewChallengeForPublicResponseV215(challenge: HumanHandoffChallengeV1) {
  return {
