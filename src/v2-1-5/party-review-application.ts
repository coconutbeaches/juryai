...
import { validatePartyReviewStateV215, type PartyReviewStateV215 } from './party-review-state.js';
import {
  ENVELOPE_COMMAND_VERSION_V215,
...
import {
  ...
  type EnvelopeCeremonyCommandV215,
  type RecordPartyConfirmationOperationV215,
  type ReopenOwnFormationOperationV215,
} from './envelope-ceremony.js';
import {
  derivePartyConfirmationEligibilityV215,
  derivePartyReviewStateV215,
} from './party-review-state.js';

export const PARTY_REVIEW_PROTECTED_ACTION_VERSION_V215 =
  V215_SPEC.contracts.protected_action_version;

type ProtectedCeremonyOperationV215 =
  RecordPartyConfirmationOperationV215 | ReopenOwnFormationOperationV215;

export interface PartyReviewProtectedActionPayloadV215 {
  protected_action_version: typeof PARTY_REVIEW_PROTECTED_ACTION_VERSION_V215;
  review_state_hash: string;
  party_readback_hash: string;
  ceremony_command: EnvelopeCeremonyCommandV215 & {
    operation: ProtectedCeremonyOperationV215;
  };
}

export interface PreparePartyReviewChallengeInputV215 {
...
export type PreparePartyReviewChallengeResultV215 =
  | {
      status: 'prepared';
      party_id: PartyIdV215;
      review_state: PartyReviewStateV215;
      challenge: HumanHandoffChallengeV1;
      action_payload: PartyReviewProtectedActionPayloadV215;
    }
  | {
      status: 'rejected';
...
export type ExecutePartyReviewProtectedActionResultV215 =
  | {
      status: 'applied';
      party_id: PartyIdV215;
      envelope: CaseEnvelopeV215;
      ...
      prior_review_state: PartyReviewStateV215;
      resulting_review_state: PartyReviewStateV215;
    }
  | {
      status: 'rejected';
      reason_code:
...
function stateBinding(envelope: CaseEnvelopeV215, partyId: PartyIdV215) {
...
function operationFor(
  input: PreparePartyReviewChallengeInputV215,
): ProtectedCeremonyOperationV215 | null {
...
export function validatePartyReviewProtectedActionPayloadV215(
  value: unknown,
  ...
  if (
    ...
    payload.protected_action_version !== PARTY_REVIEW_PROTECTED_ACTION_VERSION_V215 ||
  ...
  if (
    command.command_version !== ENVELOPE_COMMAND_VERSION_V215 ||
...
export function preparePartyReviewChallengeV215(
  input: PreparePartyReviewChallengeInputV215,
): PreparePartyReviewChallengeResultV215 {
  ...
  if (
    ...
    !derivePartyConfirmationEligibilityV215(input.envelope, partyId).eligible
  ) {
    return {
      status: 'rejected',
  ...
  const command = ceremonyCommandForV215(input.envelope, input.ids.command_id, operation);
  const dryRun = applyEnvelopeCeremonyCommandV215({
    envelope: input.envelope,
    command,
    execution_authority: partyAuthorityV215(input.envelope, partyId, 'first_party_human'),
  });
  ...
  const reviewState = derivePartyReviewStateV215(input.envelope, partyId);
  if (validatePartyReviewStateV215(reviewState).length > 0) {
    return {
      status: 'rejected',
      reason_code: 'invalid_transition',
  ...
  const actionPayload: PartyReviewProtectedActionPayloadV215 = {
    protected_action_version: PARTY_REVIEW_PROTECTED_ACTION_VERSION_V215,
    review_state_hash: reviewState.review_state_hash,
    party_readback_hash: reviewState.party_readback_hash,
    ceremony_command: command as PartyReviewProtectedActionPayloadV215['ceremony_command'],
  };
  const binding = stateBinding(input.envelope, partyId);
  if (
    !binding ||
    !validatePartyReviewProtectedActionPayloadV215(actionPayload, input.requested_action)
  ) {
    return {
      status: 'rejected',
      reason_code: 'invalid_transition',
...
export function executePartyReviewProtectedActionV215(input: {
  envelope: CaseEnvelopeV215;
  authenticated_subject_id: string;
  challenge: HumanHandoffChallengeV1;
  action_payload: PartyReviewProtectedActionPayloadV215;
...
}): ExecutePartyReviewProtectedActionResultV215 {
  const partyId = partyForSubject(input.envelope, input.authenticated_subject_id);
  ...
  if (!validatePartyReviewProtectedActionPayloadV215(input.action_payload, input.expected_action)) {
    return {
      status: 'rejected',
  ...
  const priorReviewState = derivePartyReviewStateV215(input.envelope, partyId);
  if (
  ...
  return {
    ...
    resulting_review_state: derivePartyReviewStateV215(applied.envelope, partyId),
  };
}

export interface PartyReviewPersistencePortV215 {
  getPartyReview(input: {
    dispute_id: string;
    authenticated_subject_id: string;
  }): Promise<PartyReviewStateV215 | null>;
  issuePartyReviewChallenge(input: {
    ...
    | { status: 'issued'; challenge: HumanHandoffChallengeV1; review_state: PartyReviewStateV215 }
  ...
  executePartyReviewAction(input: {
    ...
    | { status: 'applied'; review_state: PartyReviewStateV215 }
    | { status: 'rejected'; reason_code: string; message: string }
  >;
}

export interface PartyReviewApplicationV215 {
  getReview(caseId: string): Promise<PartyReviewStateV215 | null>;
...
export function createPartyReviewApplicationV215(input: {
  authenticated_subject_id: string;
  repository: PartyReviewPersistencePortV215;
