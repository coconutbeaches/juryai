...
import {
  ...
  PARTY_FORMATION_READBACK_VERSION_V215,
  type CaseEnvelopeV215,
  type PartyIdV215,
} from './case-envelope.js';
import { deriveFormationReadinessV215 } from './formation-readiness.js';
import {
  currentPartyConfirmationV215,
  projectPartyFormationV215,
  renderPartyFormationReadbackV215,
  type PartyFormationReadbackV215,
  type PartyScopedFormationProjectionV215,
} from './party-projection.js';

export const PARTY_REVIEW_STATE_VERSION_V215 = V215_SPEC.contracts.party_review_state_version;

export interface PartyReviewStateV215 {
  review_state_version: typeof PARTY_REVIEW_STATE_VERSION_V215;
  dispute_id: string;
  party_id: PartyIdV215;
  formation_projection_version: typeof PARTY_FORMATION_PROJECTION_VERSION_V215;
  party_projection_hash: string;
  party_visible_version: number;
  formation_readback_version: typeof PARTY_FORMATION_READBACK_VERSION_V215;
  party_readback_hash: string;
  formation_epoch: number;
  formation_projection: PartyScopedFormationProjectionV215;
  formation_readback: PartyFormationReadbackV215;
...
function reviewHashProjection(value: Omit<PartyReviewStateV215, 'review_state_hash'>): JsonValue {
  return cloneCanonical(value) as unknown as JsonValue;
}

export function hashPartyReviewStateV215(
  value: Omit<PartyReviewStateV215, 'review_state_hash'>,
): string {
  return sha256(canonicalSerialize(reviewHashProjection(value)));
}

export function derivePartyReviewStateV215(
  envelope: CaseEnvelopeV215,
  partyId: PartyIdV215,
): PartyReviewStateV215 {
  const formationProjection = projectPartyFormationV215(envelope, partyId);
  const formationReadback = renderPartyFormationReadbackV215(envelope, partyId);
  const cursor = envelope.control.party_views[partyId];
  const reviewWithoutHash: Omit<PartyReviewStateV215, 'review_state_hash'> = {
    review_state_version: PARTY_REVIEW_STATE_VERSION_V215,
    dispute_id: envelope.control.case_id,
    party_id: partyId,
    formation_projection_version: PARTY_FORMATION_PROJECTION_VERSION_V215,
    party_projection_hash: cursor.party_projection_hash,
    party_visible_version: cursor.party_visible_version,
    formation_readback_version: PARTY_FORMATION_READBACK_VERSION_V215,
    ...
    own_confirmation_state:
      currentPartyConfirmationV215(envelope, partyId) === null ? 'unconfirmed' : 'confirmed',
    shared_readiness: deriveFormationReadinessV215(envelope).ready_for_bilateral_lock
      ? 'ready_for_lock'
      : 'not_ready',
  };
  return cloneCanonical({
    ...reviewWithoutHash,
    review_state_hash: hashPartyReviewStateV215(reviewWithoutHash),
  });
}
...
export function validatePartyReviewStateV215(value: unknown): ContractIssue[] {
  ...
  if (record.review_state_version !== PARTY_REVIEW_STATE_VERSION_V215) {
    issues.push(
      issue(
  ...
  if (
    record.formation_projection_version !== PARTY_FORMATION_PROJECTION_VERSION_V215 ||
    record.formation_readback_version !== PARTY_FORMATION_READBACK_VERSION_V215
  ) {
    issues.push(
      issue(
        'party_review_state_frozen_contract',
        '$.formation_projection_version',
        'Party review state must reference the frozen V2.1.5 formation contracts.',
      ),
    );
  }
  if (
  ...
  try {
    ...
    const expectedReadbackDocument = canonicalSerialize({
      readback_version: PARTY_FORMATION_READBACK_VERSION_V215,
...
export function derivePartyConfirmationEligibilityV215(
  envelope: CaseEnvelopeV215,
  partyId: PartyIdV215,
): PartyConfirmationEligibilityV1 {
  const readiness = deriveFormationReadinessV215(envelope);
  ...
  if (currentPartyConfirmationV215(envelope, partyId) !== null) {
