...
import {
  ...
  assertQualifiedCompiler,
} from './compiler-contract.js';
/** Private PostgreSQL persistence for V2.1.5 formation and review commands. */
...
import { COMPILER_INPUT_TEMPLATE_VERSION } from '../webmcp/core-v0-3/compiler-contract.js';
import {
  HASH_PATTERN_V215,
  ID_PATTERN_V215,
...
import {
  ...
  type FormationSourceAuditRecordV215,
  type FormationSubmissionAuditRecordV215,
} from './formation-persistence.js';
import {
  applyExternalRelaySubmissionV215,
  rebaseExternalRelaySubmissionV215,
  type ExternalRelaySubmissionV215,
} from './external-relay-submission.js';
import {
  FORMATION_PERSISTENCE_CONTRACT_VERSION_V215,
  FORMATION_PERSISTENCE_SCHEMA_V215,
  type ActiveFormationContextV215,
  type CommitCeremonyResultV215,
  type CommitControlledDisclosureInputV215,
  type CommitDisclosureReviewAcknowledgmentInputV215,
  type CommitExternalRelaySubmissionInputV215,
  type CommitExternalRelaySubmissionResultV215,
  type CommitFinalConfirmationInputV215,
  type FormationPartyPersistenceContextV215,
  type StoredFormationDisputeV215,
} from './formation-persistence.js';
import {
  executePartyReviewProtectedActionV215,
  preparePartyReviewChallengeV215,
  validatePartyReviewProtectedActionPayloadV215,
  type PartyReviewPersistencePortV215,
  type PartyReviewProtectedActionPayloadV215,
} from './party-review-application.js';
import { derivePartyReviewStateV215 } from './party-review-state.js';
import {
  V215_CONTRACT_PAIR_READINESS_PATTERN,
  V215_EXTERNAL_SUBMISSION_READINESS_PATTERN,
  V215_PROTECTED_ACTION_READINESS_PATTERN,
} from './postgres-readiness-contract.js';

const SCHEMA = FORMATION_PERSISTENCE_SCHEMA_V215;
...
export interface PostgresDisclosureReviewRepositoryOptionsV215 extends PoolConfig {
  pool?: Pool;
  clock?: { now: () => string };
  ids?: {
    next: (kind: PartyReviewIdentityKindV215, partyId: PartyIdV215) => string;
    public_reference: () => string;
  };
}

export type PartyReviewIdentityKindV215 =
  | 'challenge'
  | 'command'
  | 'confirmation'
  | 'confirmation_event'
...
function canonicalId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !ID_PATTERN_V215.test(value)) {
    throw new TypeError(`${label} is not canonical.`);
  }
  return value;
}

function partyId(value: unknown): PartyIdV215 {
...
function hash(value: unknown, label: string): string {
  if (typeof value !== 'string' || !HASH_PATTERN_V215.test(value)) {
...
function decodeActionPayload(value: unknown): PartyReviewProtectedActionPayloadV215 {
  const payload = cloneCanonical(value as PartyReviewProtectedActionPayloadV215);
  if (!validatePartyReviewProtectedActionPayloadV215(payload)) {
    throw new TypeError('Stored protected action payload failed its canonical contract.');
  }
  return payload;
}

function defaultReviewIdentity(kind: PartyReviewIdentityKindV215, party: PartyIdV215): string {
  const prefix: Record<PartyReviewIdentityKindV215, string> = {
...
function reviewActionIds(
  options: NonNullable<PostgresDisclosureReviewRepositoryOptionsV215['ids']>,
...
function sameCreationIdentity(requested: CaseEnvelopeV215, stored: CaseEnvelopeV215): boolean {
  const bindingIdentity = (envelope: CaseEnvelopeV215, party: PartyIdV215) => {
...
function decodeReplay(value: unknown): FormationReplayRecordV215 {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new TypeError('Replay record is invalid.');
  }
  const record = cloneCanonical(value as FormationReplayRecordV215);
  ...
  const idArray = (candidate: unknown): candidate is string[] =>
    ...
    candidate.every((id) => typeof id === 'string' && ID_PATTERN_V215.test(id));
  if (
    ...
    !HASH_PATTERN_V215.test(record.request_fingerprint) ||
    !Number.isSafeInteger(record.recorded_at_ms) ||
    Object.keys(responseRecord).sort().join(',') !== responseKeys.join(',') ||
    record.response.persistence_contract_version !== FORMATION_PERSISTENCE_CONTRACT_VERSION_V215 ||
    record.response.dispute_id !== record.dispute_id ||
    record.response.party_id !== record.party_id ||
    !ID_PATTERN_V215.test(record.response.submission_id) ||
    !ID_PATTERN_V215.test(record.response.source_turn_id) ||
    ...
    !HASH_PATTERN_V215.test(record.response.resulting_internal_envelope_hash) ||
    !Number.isSafeInteger(record.response.resulting_party_visible_version) ||
    record.response.resulting_party_visible_version < 1 ||
    !HASH_PATTERN_V215.test(record.response.resulting_party_projection_hash)
  ) {
    throw new TypeError('Replay record is invalid.');
  }
...
function decodeStored(row: StoredFormationRow): StoredFormationDisputeV215 {
  const envelope = cloneCanonical(row.envelope as CaseEnvelopeV215);
  assertValidCaseEnvelopeV215(envelope);
  ...
  if (
    ...
    throw new TypeError('Stored envelope identity disagrees with canonical V2.1.5 state.');
  }
  return {
    envelope,
...
function partyForSubject(envelope: CaseEnvelopeV215, subject: string): PartyIdV215 | null {
...
export class PostgresDisclosureReviewRepositoryV215 {
  readonly #pool: Pool;
  readonly #ownsPool: boolean;
  readonly #issuedContexts = new WeakSet<object>();
  readonly #clock: { now: () => string };
  readonly #ids: NonNullable<PostgresDisclosureReviewRepositoryOptionsV215['ids']>;

  constructor(options: PostgresDisclosureReviewRepositoryOptionsV215) {
    this.#clock = options.clock ?? { now: () => new Date().toISOString() };
    this.#ids =
      options.ids ??
      ({
        next: defaultReviewIdentity,
        public_reference: () => `JURYAI-${randomBytes(6).toString('hex').toUpperCase()}`,
      } satisfies NonNullable<PostgresDisclosureReviewRepositoryOptionsV215['ids']>);
    if (options.pool) {
  ...
  async assertReady(): Promise<void> {
    ...
    if (result.rows[0]?.ready !== true) {
      throw new Error('V2.1.5 disclosure-review persistence migration is incomplete.');
    }
  }
  ...
  async getPartyReview(input: {
  ...
  }): ReturnType<PartyReviewPersistencePortV215['getPartyReview']> {
    ...
    const result = await this.#pool.query<{ envelope: unknown }>(
      `select envelope from ${SCHEMA}.formation_disputes
        where dispute_id = $1 and schema_version = '${V215_SPEC.identity.envelope_schema_version}'
    ...
    return party ? derivePartyReviewStateV215(envelope, party) : null;
  }

  async issuePartyReviewChallenge(
    input: Parameters<PartyReviewPersistencePortV215['issuePartyReviewChallenge']>[0],
  ): ReturnType<PartyReviewPersistencePortV215['issuePartyReviewChallenge']> {
    const id = disputeId(input.dispute_id);
    const subject = canonicalId(input.authenticated_subject_id, 'authenticated_subject_id');
    return this.#transaction(async (client) => {
      const selected = await client.query<{ envelope: unknown }>(
        `select envelope from ${SCHEMA}.formation_disputes
          where dispute_id = $1 and schema_version = '${V215_SPEC.identity.envelope_schema_version}' for update`,
        [id],
      );
      ...
      const prepared = preparePartyReviewChallengeV215({
        envelope,
  ...
  async executePartyReviewAction(
    input: Parameters<PartyReviewPersistencePortV215['executePartyReviewAction']>[0],
  ): ReturnType<PartyReviewPersistencePortV215['executePartyReviewAction']> {
    const id = disputeId(input.dispute_id);
    const subject = canonicalId(input.authenticated_subject_id, 'authenticated_subject_id');
    ...
    return this.#transaction(async (client) => {
      ...
      const executed = executePartyReviewProtectedActionV215({
  ...
  async findById(idInput: string): Promise<StoredFormationDisputeV215 | null> {
  ...
  async listActiveContextsForPrincipal(
    subjectInput: string,
  ): Promise<ActiveFormationContextV215[]> {
    ...
    const result = await this.#pool.query(
      `select ${selectedFormationColumns('dispute')},
        ...
        where dispute.schema_version = '${V215_SPEC.identity.envelope_schema_version}'
  ...
  async resolvePartyContext(
    ...
    const result = await this.#pool.query(
      `select ${selectedFormationColumns('dispute')},
        ...
        where dispute.dispute_id = $1
          and dispute.schema_version = '${V215_SPEC.identity.envelope_schema_version}'
    ...
    const cursor = stored.envelope.control.party_views[party];
    const context: FormationPartyPersistenceContextV215 = Object.freeze({
      dispute_id: id,
      party_id: party,
      authenticated_subject_id: subject,
      internal_envelope_version: stored.internal_envelope_version,
  ...
  async createDispute(
    envelopeInput: CaseEnvelopeV215,
    ...
    return this.#transaction(async (client) => {
      ...
      if (!sameCreationIdentity(envelope, stored.envelope)) {
        throw new TypeError('A different V2.1.5 creation identity already uses this dispute id.');
      }
  ...
  async commitExternalRelaySubmission(
    ...
    // Private append-only audit data, never a public projection or mutable authority.
    // Retain the exact original compiler view even if hidden opponent state rebases.
    const artifact = cloneCanonical(input.compiler_artifact);
    const { registry_entry: registry, run } = artifact;
    assertQualifiedCompiler(registry);
    const commitment = input.submission.compiler_run;
    const reproduced = buildCompileRunRecord(run.input, run.output, run);
    if (
      registry.version.schema_version !== COMPILER_CONTRACT_VERSION ||
      registry.version.taxonomy_version !== V215_SPEC.compiler.taxonomy_version ||
      run.input_template_version !== COMPILER_INPUT_TEMPLATE_VERSION ||
      registry.compiler_version_id !== commitment.compiler_version_id ||
      run.compiler_version_id !== commitment.compiler_version_id ||
      run.compile_run_id !== commitment.compile_run_id ||
      run.case_id !== input.context.dispute_id ||
      run.turn_id !== input.submission.source_turn.turn_id ||
      run.input_hash !== commitment.input_hash ||
      sha256(canonicalSerialize(run.output)) !== commitment.output_hash ||
      canonicalSerialize(run.input.turn.payload) !==
        canonicalSerialize(input.submission.source_turn.payload) ||
      canonicalSerialize(run) !== canonicalSerialize(reproduced) ||
      reproduced.contract_issues.length !== 0
    ) {
      throw new TypeError('Compiler artifact does not reproduce the committed V2.1.5 run.');
    }
    ...
    return this.#transaction(async (client) => {
      ...
      if (
        ...
        const rebased = rebaseExternalRelaySubmissionV215(submission, current.envelope);
      ...
      const applied = applyExternalRelaySubmissionV215({
        ...
        submission,
        execution_authority: partyAuthorityV215(current.envelope, party, 'external_relay'),
      });
      ...
      const sourceRecord: FormationSourceAuditRecordV215 = {
      ...
      const submissionRecord: FormationSubmissionAuditRecordV215 = {
        dispute_id: current.envelope.control.case_id,
        party_id: party,
        submission_id: submission.submission_id,
      ...
      const compilerRecord: FormationCompilerRunAuditRecordV215 = {
        persistence_contract_version: FORMATION_PERSISTENCE_CONTRACT_VERSION_V215,
      ...
      const replayRecord: FormationReplayRecordV215 = {
        dispute_id: current.envelope.control.case_id,
        party_id: party,
        client_turn_id: submission.source_turn.client_turn_id,
  ...
  async commitControlledDisclosure(
    ...
    return this.#transaction(async (client) => {
      ...
      const applied = applyEnvelopeCeremonyCommandV215({
        envelope: current.envelope,
        command: ceremonyCommandForV215(current.envelope, input.command_id, {
  ...
  async commitDisclosureReviewAcknowledgment(
    ...
    return this.#transaction(async (client) => {
      ...
      const command = ceremonyCommandForV215(current.envelope, input.command_id, {
      ...
      const applied = applyEnvelopeCeremonyCommandV215({
        envelope: current.envelope,
        command,
        execution_authority: partyAuthorityV215(current.envelope, partyId, 'first_party_human'),
      });
      if (applied.status === 'rejected') {
        return {
          status: 'domain_rejected',
  ...
  async commitFinalConfirmation(
    input: CommitFinalConfirmationInputV215,
  ): Promise<CommitCeremonyResultV215> {
    const id = disputeId(input.dispute_id);
    canonicalId(input.command_id, 'command_id');
    safeInteger(input.expected_internal_envelope_version, 'expected version', 1);
