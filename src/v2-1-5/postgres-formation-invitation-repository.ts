...
const AUTHORITY_BRAND: unique symbol = Symbol('juryai-v2.1.5-production-invitation');

export interface TrustedProductionInvitationAuthorityV215 {
  readonly authority_kind: string;
  readonly [AUTHORITY_BRAND]: true;
}

const TRUSTED_PRODUCTION_INVITATION_AUTHORITY_V215: TrustedProductionInvitationAuthorityV215 =
  Object.freeze({
    authority_kind: V215_SPEC.authority.production_invitation_authority_kind,
    [AUTHORITY_BRAND]: true as const,
  });

/** Server composition obtains this capability only after the fail-closed switch check. */
export function productionInvitationAuthorityV215(
  enabled: boolean,
): TrustedProductionInvitationAuthorityV215 | null {
  return enabled ? TRUSTED_PRODUCTION_INVITATION_AUTHORITY_V215 : null;
}

function authorized(value: unknown): value is TrustedProductionInvitationAuthorityV215 {
  return value === TRUSTED_PRODUCTION_INVITATION_AUTHORITY_V215;
}
...
export interface PostgresFormationInvitationRepositoryOptionsV215 extends PoolConfig {
  pool?: Pool;
...
export type RedeemFormationInvitationResultV215 =
...
function decodeEnvelopeRow(row: FormationDisputeRow): CaseEnvelopeV215 {
...
export class PostgresFormationInvitationRepositoryV215 {
  readonly #pool: Pool;
  ...
  constructor(options: PostgresFormationInvitationRepositoryOptionsV215) {
  ...
  async assertReady(): Promise<void> {
    const result = await this.#pool.query<{ ready: boolean }>(
      ...
      [SCHEMA, V215_CONTRACT_PAIR_READINESS_PATTERN, V215_EXTERNAL_SUBMISSION_READINESS_PATTERN],
    );
    if (result.rows[0]?.ready !== true) {
      throw new Error('V2.1.5 invitation persistence is unavailable.');
    }
  }
  ...
  async issueInvitation(input: {
    authority: TrustedProductionInvitationAuthorityV215 | null;
    dispute_id: string;
  ...
  async redeemInvitation(input: {
  ...
  }): Promise<RedeemFormationInvitationResultV215> {
    if (!authorized(input.authority)) return invitationUnavailableResultV21();
    ...
    try {
      return await this.#transaction(async (client) => {
        ...
        const command = ceremonyCommandForV215(envelope, `command_invitation_${suffix}`, {
          type: 'bind_party',
          party_slot: 'party_b',
        ...
        await client.query(`insert into ${SCHEMA}.formation_commands (record) values ($1::jsonb)`, [
          canonicalSerialize({
            ...
            authority_type: V215_SPEC.authority.trusted_system_authority_kind,
