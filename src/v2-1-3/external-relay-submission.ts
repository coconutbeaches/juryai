...
import {
  ...
  type FormationChallengeV213,
  type FormationClarificationV213,
  type PartyIdV213,
  type SourceSpanCommitmentV213,
  type SourceTurnV213,
} from './case-envelope.js';
import { validateCaseEnvelopeV213 } from './contract-validator.js';
import { refreshPartyViewCursorsV213 } from './envelope-ceremony.js';
import { authoritativeFormationExplanatoryStateV213 } from './formation-readiness.js';

export interface ExternalRelaySubmissionIntentV213 {
  intent_version: typeof EXTERNAL_RELAY_SUBMISSION_INTENT_VERSION_V213;
...
export interface SemanticAssertionCandidateEffectV213 {
...
export interface ClarificationRequestEffectV213 {
...
export interface ChallengeCandidateEffectV213 {
...
export interface ChallengeResponseCandidateEffectV213 {
  type: 'challenge_response_candidate';
  challenge_id: string;
  statement: string;
  spans: TurnSpan[];
  semantic_correction: SemanticAssertionCandidateEffectV213 | null;
}

export type ExternalRelayEffectCandidateV213 =
  | SemanticAssertionCandidateEffectV213
  | ClarificationRequestEffectV213
  | ChallengeCandidateEffectV213
  | ChallengeResponseCandidateEffectV213;

export interface PreparedSemanticAssertionEffectV213 extends SemanticAssertionCandidateEffectV213 {
  position_id: string;
}

export interface PreparedClarificationRequestEffectV213 extends ClarificationRequestEffectV213 {
  clarification_id: string;
}

export interface PreparedChallengeEffectV213 extends ChallengeCandidateEffectV213 {
  challenge_id: string;
}

export interface PreparedChallengeResponseEffectV213 extends Omit<
  ChallengeResponseCandidateEffectV213,
  'semantic_correction'
> {
  response_id: string;
  semantic_correction: PreparedSemanticAssertionEffectV213 | null;
}

export type PreparedExternalRelayEffectV213 =
  | PreparedSemanticAssertionEffectV213
  | PreparedClarificationRequestEffectV213
  | PreparedChallengeEffectV213
  | PreparedChallengeResponseEffectV213;

export interface ExternalRelayCompilerIdentityV213 {
  compile_run_id: string;
  compiler_version_id: string;
  party_projection_contract_version: typeof PARTY_FORMATION_PROJECTION_VERSION_V213;
  input_hash: string;
  output_hash: string;
}

export interface ExternalRelaySubmissionV213 {
  submission_version: typeof EXTERNAL_RELAY_SUBMISSION_VERSION_V213;
  ...
  compiler_run: ExternalRelayCompilerIdentityV213;
  effects: PreparedExternalRelayEffectV213[];
}

export interface ExternalRelayCanonicalIdsV213 {
  submission_id: string;
...
const RELAY_RUNTIME_BRAND_V213: unique symbol = Symbol('juryai-relay-runtime-v2.1.3');

export interface TrustedExternalRelayRuntimeV213 {
  readonly source_channel: SourceChannel;
  readonly relaying_agent: string | null;
  readonly received_at: string;
  readonly payload_commitment_salt: string;
  readonly ids: ExternalRelayCanonicalIdsV213;
  readonly [RELAY_RUNTIME_BRAND_V213]: true;
}

export const TRUSTED_EXTERNAL_RELAY_BRIDGE_V213 = Object.freeze({
  authority_kind: 'trusted_external_relay_bridge_v2_1_3' as const,
});

export function trustedExternalRelayRuntimeV213(
  bridge: typeof TRUSTED_EXTERNAL_RELAY_BRIDGE_V213,
  input: Omit<TrustedExternalRelayRuntimeV213, typeof RELAY_RUNTIME_BRAND_V213>,
): TrustedExternalRelayRuntimeV213 {
  if (bridge !== TRUSTED_EXTERNAL_RELAY_BRIDGE_V213) {
    throw new TypeError('Trusted external relay bridge is required.');
  }
  return Object.freeze({ ...input, [RELAY_RUNTIME_BRAND_V213]: true as const });
}

function isTrustedExternalRelayRuntimeV213(
  value: unknown,
): value is TrustedExternalRelayRuntimeV213 {
  return (
    typeof value === 'object' &&
    value !== null &&
    RELAY_RUNTIME_BRAND_V213 in value &&
    (value as TrustedExternalRelayRuntimeV213)[RELAY_RUNTIME_BRAND_V213] === true
  );
}

export type PrepareExternalRelaySubmissionResultV213 =
  | { status: 'prepared'; submission: ExternalRelaySubmissionV213 }
...
function candidateEffectShape(effect: unknown): effect is ExternalRelayEffectCandidateV213 {
    ...
    case 'challenge_response_candidate':
      return (
        ...
        (typed.semantic_correction === null ||
          (typed.semantic_correction as ExternalRelayEffectCandidateV213).type ===
...
function preparedEffectShape(effect: unknown): effect is PreparedExternalRelayEffectV213 {
  if (typeof effect !== 'object' || effect === null || !('type' in effect)) return false;
  const typed = effect as Record<string, unknown>;
  switch (typed.type) {
    case 'semantic_assertion_candidate':
    ...
    case 'challenge_response_candidate':
      return (
        ...
        (typed.semantic_correction === null ||
          (typed.semantic_correction as PreparedExternalRelayEffectV213).type ===
            'semantic_assertion_candidate') &&
        typeof typed.challenge_id === 'string' &&
...
function idsHaveScope(ids: ExternalRelayCanonicalIdsV213, partyId: PartyIdV213): boolean {
  return (
    ID_PATTERN_V213.test(ids.submission_id) &&
    ids.submission_id.startsWith(`submission_${partyId}_`) &&
    isPartyScopedIdV213('turn', partyId, ids.source_turn_id) &&
    ids.position_ids.every((id) => isPartyScopedIdV213('position', partyId, id)) &&
    ids.clarification_ids.every((id) => isPartyScopedIdV213('clarification', partyId, id)) &&
    ids.challenge_ids.every((id) => isPartyScopedIdV213('challenge', partyId, id)) &&
    ids.challenge_response_ids.every((id) => isPartyScopedIdV213('challenge_response', partyId, id))
  );
}

function requiredIdCounts(effects: readonly ExternalRelayEffectCandidateV213[]) {
  let positions = 0;
...
function replyTargetVisibleToPartyV213(
...
export function prepareExternalRelaySubmissionV213(input: {
  envelope: CaseEnvelopeV213;
  execution_authority: AuthenticatedPartyAuthorityV213;
  intent: ExternalRelaySubmissionIntentV213;
  runtime: TrustedExternalRelayRuntimeV213;
  compiler_run: ExternalRelayCompilerIdentityV213;
  effects: ExternalRelayEffectCandidateV213[];
}): PrepareExternalRelaySubmissionResultV213 {
  const {
    envelope,
  ...
  if (
    ...
    !isTrustedExternalRelayRuntimeV213(runtime)
  ) {
    return {
      status: 'rejected',
  ...
  if (
    ...
    !Array.isArray(effects) ||
    !effects.every(candidateEffectShape) ||
    intent.intent_version !== EXTERNAL_RELAY_SUBMISSION_INTENT_VERSION_V213 ||
    !Number.isSafeInteger(intent.expected_party_visible_version) ||
    intent.expected_party_visible_version < 1 ||
    !HASH_PATTERN_V213.test(intent.expected_party_projection_hash) ||
    ...
    !intent.in_reply_to.every(
      (target) =>
        ID_PATTERN_V213.test(target) &&
        replyTargetVisibleToPartyV213(envelope, authority.party_id, target),
    ) ||
  ...
  let responseIndex = 0;
  const preparedEffects: PreparedExternalRelayEffectV213[] = effects.map((effect) => {
    switch (effect.type) {
      case 'semantic_assertion_candidate':
        return {
  ...
  const sourceTurn: SourceTurnV213 = {
  ...
  return {
    status: 'prepared',
    submission: {
      submission_version: EXTERNAL_RELAY_SUBMISSION_VERSION_V213,
...
export type ExternalRelaySubmissionFailureReasonV213 =
  | 'invalid_envelope'
  | 'invalid_submission'
  | 'unauthorized_actor'
  | 'case_mismatch'
...
export interface AppliedExternalRelaySubmissionV213 {
...
export type ApplyExternalRelaySubmissionResultV213 =
  | {
      status: 'applied';
      reason_code: null;
      message: string;
      ...
      result: AppliedExternalRelaySubmissionV213;
    }
  | {
      status: 'rejected';
...
function rejected(
  envelope: CaseEnvelopeV213,
  reason: ExternalRelaySubmissionFailureReasonV213,
  message: string,
  issues: ContractIssue[] = [],
): ApplyExternalRelaySubmissionResultV213 {
  return {
    status: 'rejected',
    reason_code: reason,
    message,
...
function spanCommitments(
  ...
  source: SourceTurnV213,
  path: string,
): { commitments: SourceSpanCommitmentV213[]; issues: ContractIssue[] } {
...
function assertionFailure(
  envelope: CaseEnvelopeV213,
  partyId: PartyIdV213,
  effect: PreparedSemanticAssertionEffectV213,
  source: SourceTurnV213,
  claimedSlots: Set<string>,
  claimedTargets: Set<string>,
  path: string,
): { issue: ContractIssue | null; commitments: SourceSpanCommitmentV213[] } {
  ...
  if (
    ...
    return {
      issue: {
        code: 'v213_explicit_absence_source',
        path,
  ...
  if (
    ...
    return {
      issue: {
        code: 'v213_assertion_requirement',
        path,
  ...
  if (
    ...
    !ID_PATTERN_V213.test(effect.compiler_assertion_id) ||
    propositionTypeDescriptor(effect.proposed_type).requires_inspected_evidence
  ) {
    return {
      issue: {
        code: 'v213_assertion_semantics',
        path,
  ...
  if (envelope.positions[effect.position_id]) {
    return {
      issue: { code: 'v213_position_id_collision', path, message: 'Position id already exists.' },
      commitments: [],
    };
  }
  ...
  if (claimedSlots.has(slot)) {
    return {
      issue: {
        code: 'v213_assertion_slot_duplicate',
        path,
  ...
  if (effect.supersedes_candidate === null) {
    if (existing) {
      return {
        issue: {
          code: 'v213_live_position_slot_collision',
          path,
...
function materialOwnEdit(effect: PreparedExternalRelayEffectV213): boolean {
  return (
...
export function rebaseExternalRelaySubmissionV213(
  submission: ExternalRelaySubmissionV213,
  current: CaseEnvelopeV213,
): ExternalRelaySubmissionV213 | null {
...
export function applyExternalRelaySubmissionV213(input: {
  envelope: CaseEnvelopeV213;
  submission: ExternalRelaySubmissionV213;
  execution_authority: AuthenticatedPartyAuthorityV213;
}): ApplyExternalRelaySubmissionResultV213 {
  ...
  if (
    submission.submission_version !== EXTERNAL_RELAY_SUBMISSION_VERSION_V213 ||
    !submission.submission_id.startsWith(`submission_${partyId}_`) ||
    !ID_PATTERN_V213.test(submission.submission_id) ||
    ...
    ].every((value) => HASH_PATTERN_V213.test(value))
  ) {
    return rejected(
      envelope,
      'invalid_submission',
  ...
  if (
    !isPartyScopedIdV213('turn', partyId, submission.source_turn.turn_id) ||
  ...
  const newPositions: CanonicalSemanticPositionV213[] = [];
  const newClarifications: FormationClarificationV213[] = [];
  const newChallenges: FormationChallengeV213[] = [];
  const responseEffects: Array<{
    challenge: FormationChallengeV213;
    effect: PreparedChallengeResponseEffectV213;
    commitments: SourceSpanCommitmentV213[];
    correction: CanonicalSemanticPositionV213 | null;
  }> = [];

  const createPosition = (
    effect: PreparedSemanticAssertionEffectV213,
    path: string,
  ): CanonicalSemanticPositionV213 | ApplyExternalRelaySubmissionResultV213 => {
  ...
  for (const [index, effect] of submission.effects.entries()) {
    ...
    switch (effect.type) {
      case 'semantic_assertion_candidate': {
        if (!isPartyScopedIdV213('position', partyId, effect.position_id)) {
      ...
      case 'clarification_request': {
        ...
        if (
          ...
          !isPartyScopedIdV213('clarification', partyId, effect.clarification_id) ||
      ...
      case 'challenge_candidate': {
        ...
        if (
          ...
          !target ||
          target.attributed_party_id !== otherPartyV213(partyId) ||
          target.superseded_by !== null ||
          !submission.source_turn.in_reply_to.includes(effect.target_position_id) ||
          !isPartyScopedIdV213('challenge', partyId, effect.challenge_id) ||
      ...
      case 'challenge_response_candidate': {
        ...
        if (
          ...
          !isPartyScopedIdV213('challenge_response', partyId, effect.response_id) ||
          Object.values(envelope.challenges).some(
        ...
        let correction: CanonicalSemanticPositionV213 | null = null;
  ...
  return {
    status: 'applied',
    reason_code: null,
    message: 'V2.1.3 external relay submission applied atomically.',
    envelope: candidate,
    prior_envelope_version: envelope.control.envelope_version,
...
export function conflictTurnSummariesForPartyV213(
  envelope: CaseEnvelopeV213,
  partyId: PartyIdV213,
  limit = 3,
): ConflictTurnSummary[] {
