...
const AUTHORITY_BRAND: unique symbol = Symbol('juryai-v2.1.3-production-invitation');

export interface TrustedProductionInvitationAuthorityV213 {
  readonly authority_kind: 'trusted_v2_1_3_production_invitation';
  readonly [AUTHORITY_BRAND]: true;
}

const TRUSTED_PRODUCTION_INVITATION_AUTHORITY_V213: TrustedProductionInvitationAuthorityV213 =
  Object.freeze({
...
export function productionInvitationAuthorityV213(
  enabled: boolean,
): TrustedProductionInvitationAuthorityV213 | null {
  return enabled ? TRUSTED_PRODUCTION_INVITATION_AUTHORITY_V213 : null;
}

function authorized(value: unknown): value is TrustedProductionInvitationAuthorityV213 {
  return value === TRUSTED_PRODUCTION_INVITATION_AUTHORITY_V213;
}
...
export interface PostgresFormationInvitationRepositoryOptionsV213 extends PoolConfig {
  pool?: Pool;
...
export type RedeemFormationInvitationResultV213 =
...
function decodeEnvelopeRow(row: FormationDisputeRow): CaseEnvelopeV213 {
...
export class PostgresFormationInvitationRepositoryV213 {
  readonly #pool: Pool;
  ...
  constructor(options: PostgresFormationInvitationRepositoryOptionsV213) {
  ...
  async assertReady(): Promise<void> {
    ...
    if (result.rows[0]?.ready !== true) {
      throw new Error('V2.1.3 invitation persistence is unavailable.');
    }
  }
  ...
  async issueInvitation(input: {
    authority: TrustedProductionInvitationAuthorityV213 | null;
    dispute_id: string;
  ...
  async redeemInvitation(input: {
  ...
  }): Promise<RedeemFormationInvitationResultV213> {
    if (!authorized(input.authority)) return invitationUnavailableResultV21();
    ...
    try {
      return await this.#transaction(async (client) => {
        ...
        const command = ceremonyCommandForV213(envelope, `command_invitation_${suffix}`, {
          type: 'bind_party',
          party_slot: 'party_b',
