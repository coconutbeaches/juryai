...
import {
  ...
  type EvidenceReferenceV213,
  type PartyConfirmationReceiptV213,
  type PartyIdV213,
  type SourceSpanCommitmentV213,
} from './case-envelope.js';
import {
  derivePartyIndependentFormationCompleteV213,
  evaluateFormationRequirementV213,
  type FormationRequirementStatusV213,
} from './formation-requirements.js';

export interface PartyVisiblePositionV213 {
  position_id: string;
...
export interface PartyVisibleRequirementV213 {
  requirement_id: string;
...
export interface PartyVisibleClarificationV213 {
...
export interface PartyVisibleChallengeResponseV213 {
  response_id: string;
...
export interface PartyVisibleChallengeV213 {
  challenge_id: string;
  ...
  response: PartyVisibleChallengeResponseV213 | null;
}

export interface PartyScopedFormationProjectionV213 {
  projection_version: typeof PARTY_FORMATION_PROJECTION_VERSION_V213;
  case_id: string;
  party_id: PartyIdV213;
  ...
  own_material: {
    positions: PartyVisiblePositionV213[];
    requirements: PartyVisibleRequirementV213[];
    clarifications: PartyVisibleClarificationV213[];
    evidence: EvidenceReferenceV213[];
  };
  visible_challenges: PartyVisibleChallengeV213[];
...
export interface PartyFormationReadbackV213 {
  readback_version: typeof PARTY_FORMATION_READBACK_VERSION_V213;
  party_id: PartyIdV213;
...
function requirementsFor(
  ...
  return Object.values(envelope.requirements)
    ...
    .map((requirement) => ({
      ...
      ...evaluateFormationRequirementV213(envelope, requirement),
    }));
}
...
function evidenceFor(envelope: CaseEnvelopeV213, partyId: PartyIdV213): EvidenceReferenceV213[] {
...
function visibleChallenge(
  challenge: CaseEnvelopeV213['challenges'][string],
): PartyVisibleChallengeV213 {
  return {
...
function projectionWarnings(
  ownEvidence: readonly EvidenceReferenceV213[],
  opponentEvidence: readonly EvidenceReferenceV213[],
): string[] {
...
export function projectPartyFormationV213(
  envelope: CaseEnvelopeV213,
  partyId: PartyIdV213,
): PartyScopedFormationProjectionV213 {
  const opponentId = otherPartyV213(partyId);
  ...
  return cloneCanonical({
    projection_version: PARTY_FORMATION_PROJECTION_VERSION_V213,
    ...
    own_progress: {
      independent_formation_complete: derivePartyIndependentFormationCompleteV213(
        envelope,
        partyId,
      ),
...
export function serializePartyFormationProjectionV213(
  envelope: CaseEnvelopeV213,
  partyId: PartyIdV213,
): string {
  return canonicalSerialize(projectPartyFormationV213(envelope, partyId));
}

export function hashPartyFormationProjectionV213(
  envelope: CaseEnvelopeV213,
  partyId: PartyIdV213,
): string {
  return sha256(serializePartyFormationProjectionV213(envelope, partyId));
}

export function renderPartyFormationReadbackV213(
  envelope: CaseEnvelopeV213,
  partyId: PartyIdV213,
): PartyFormationReadbackV213 {
...
export function currentPartyConfirmationV213(
  envelope: CaseEnvelopeV213,
  partyId: PartyIdV213,
): PartyConfirmationReceiptV213 | null {
