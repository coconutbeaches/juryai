...
import { deriveFormationReadinessV213 } from './formation-readiness.js';
import {
  currentPartyConfirmationV213,
  projectPartyFormationV213,
  renderPartyFormationReadbackV213,
  type PartyFormationReadbackV213,
  type PartyScopedFormationProjectionV213,
} from './party-projection.js';

export const PARTY_REVIEW_STATE_VERSION_V213 = 'juryai-party-review-state-v1.1.0';

export interface PartyReviewStateV213 {
  review_state_version: typeof PARTY_REVIEW_STATE_VERSION_V213;
  dispute_id: string;
  party_id: PartyIdV213;
  formation_projection_version: typeof PARTY_FORMATION_PROJECTION_VERSION_V213;
  party_projection_hash: string;
  party_visible_version: number;
  formation_readback_version: typeof PARTY_FORMATION_READBACK_VERSION_V213;
  party_readback_hash: string;
  formation_epoch: number;
  formation_projection: PartyScopedFormationProjectionV213;
  formation_readback: PartyFormationReadbackV213;
...
function reviewHashProjection(value: Omit<PartyReviewStateV213, 'review_state_hash'>): JsonValue {
  return cloneCanonical(value) as unknown as JsonValue;
}

export function hashPartyReviewStateV213(
  value: Omit<PartyReviewStateV213, 'review_state_hash'>,
): string {
  return sha256(canonicalSerialize(reviewHashProjection(value)));
}

export function derivePartyReviewStateV213(
  envelope: CaseEnvelopeV213,
  partyId: PartyIdV213,
): PartyReviewStateV213 {
  const formationProjection = projectPartyFormationV213(envelope, partyId);
  const formationReadback = renderPartyFormationReadbackV213(envelope, partyId);
  const cursor = envelope.control.party_views[partyId];
  const reviewWithoutHash: Omit<PartyReviewStateV213, 'review_state_hash'> = {
    review_state_version: PARTY_REVIEW_STATE_VERSION_V213,
    dispute_id: envelope.control.case_id,
    party_id: partyId,
    formation_projection_version: PARTY_FORMATION_PROJECTION_VERSION_V213,
    party_projection_hash: cursor.party_projection_hash,
    party_visible_version: cursor.party_visible_version,
    formation_readback_version: PARTY_FORMATION_READBACK_VERSION_V213,
    ...
    own_confirmation_state:
      currentPartyConfirmationV213(envelope, partyId) === null ? 'unconfirmed' : 'confirmed',
    shared_readiness: deriveFormationReadinessV213(envelope).ready_for_bilateral_lock
      ? 'ready_for_lock'
      : 'not_ready',
  };
  return cloneCanonical({
    ...reviewWithoutHash,
    review_state_hash: hashPartyReviewStateV213(reviewWithoutHash),
  });
}
...
export function validatePartyReviewStateV213(value: unknown): ContractIssue[] {
  ...
  if (record.review_state_version !== PARTY_REVIEW_STATE_VERSION_V213) {
    issues.push(
      issue(
  ...
  if (
    record.formation_projection_version !== PARTY_FORMATION_PROJECTION_VERSION_V213 ||
    record.formation_readback_version !== PARTY_FORMATION_READBACK_VERSION_V213
  ) {
    issues.push(
      issue(
        'party_review_state_frozen_contract',
        '$.formation_projection_version',
        'Party review state must reference the frozen V2.1.3 formation contracts.',
      ),
    );
  }
  if (
  ...
  try {
    ...
    const expectedReadbackDocument = canonicalSerialize({
      readback_version: PARTY_FORMATION_READBACK_VERSION_V213,
...
export function derivePartyConfirmationEligibilityV213(
  envelope: CaseEnvelopeV213,
  partyId: PartyIdV213,
): PartyConfirmationEligibilityV1 {
  const readiness = deriveFormationReadinessV213(envelope);
  ...
  if (currentPartyConfirmationV213(envelope, partyId) !== null) {
