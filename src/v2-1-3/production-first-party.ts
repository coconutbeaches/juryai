...
import { disclosureReviewClosureCurrentV213 } from './disclosure-review.js';
import { currentDisclosureReviewAcknowledgmentV213 } from './disclosure-review.js';
import type {
  CommitCeremonyResultV213,
  CommitDisclosureReviewAcknowledgmentInputV213,
  CommitFinalConfirmationInputV213,
  StoredFormationDisputeV213,
} from './formation-persistence.js';
import {
  createPartyReviewApplicationV213,
  type PartyReviewApplicationV213,
  type PartyReviewPersistencePortV213,
} from './party-review-application.js';
import type { PartyReviewStateV213 } from './party-review-state.js';
import {
...
import type {
  ...
  TrustedProductionInvitationAuthorityV213,
} from './postgres-formation-invitation-repository.js';

export const PRODUCTION_PARTY_REVIEW_POLICY_PROFILE_ID_V213 =
  'juryai-v2.1.2-production-party-review-hhc3';
export const FIRST_PARTY_REVIEW_PAGE_VERSION_V213 = 'juryai-v2.1.3-first-party-review-page-v1.0.0';

const PRODUCTION_PARTY_REVIEW_POLICY_V213: IntentAssuranceProtocolProfileV1 = {
  policy_version: INTENT_ASSURANCE_POLICY_VERSION_V1,
  profile_id: PRODUCTION_PARTY_REVIEW_POLICY_PROFILE_ID_V213,
...
export interface ProductionFirstPartyRepositoryV213 extends PartyReviewPersistencePortV213 {
...
export interface ProductionFirstPartyServiceV213 {
  issueInvitation(input: {
    dispute_id: string;
    intended_account_email: string;
  }): ReturnType<PostgresFormationInvitationRepositoryV213['issueInvitation']>;
  redeemInvitation(input: {
    opaque_token: string;
    authenticated_email: string;
  }): ReturnType<PostgresFormationInvitationRepositoryV213['redeemInvitation']>;
  getReview(disputeId: string): Promise<PartyReviewStateV213 | null>;
  getReviewPage(disputeId: string): Promise<ProductionFirstPartyReviewPageV213 | null>;
  acknowledgeDisclosureReview(disputeId: string): Promise<CommitCeremonyResultV213>;
...
export interface ProductionFirstPartyReviewPageV213 {
  review_page_version: typeof FIRST_PARTY_REVIEW_PAGE_VERSION_V213;
  review: PartyReviewStateV213;
  workflow_phase: CaseEnvelopeV213['control']['workflow_state'];
  disclosure_state: CaseEnvelopeV213['control']['disclosure_state'];
  ...
  disclosure_review_acknowledgment_statement: typeof DISCLOSURE_REVIEW_ACKNOWLEDGMENT_STATEMENT_V213;
}
...
function resolvedPolicy(action: PartyReviewProtectedActionV1) {
  const decision = resolveIntentAssurancePolicyDecisionV1(
    action,
    PRODUCTION_PARTY_REVIEW_POLICY_V213,
    TRUSTED_INTENT_ASSURANCE_POLICY_RESOLVER_V1,
  );
...
function unavailable(): CommitCeremonyResultV213 {
  return { status: 'unauthorized' };
}

export function createProductionFirstPartyServiceV213(input: {
  enabled: boolean;
  authenticated_subject_id: string;
  repository: ProductionFirstPartyRepositoryV213;
  invitations: PostgresFormationInvitationRepositoryV213;
  invitation_authority: TrustedProductionInvitationAuthorityV213 | null;
  clock?: { now: () => string };
}): ProductionFirstPartyServiceV213 {
  const now = input.clock?.now ?? (() => new Date().toISOString());
  const review: PartyReviewApplicationV213 = createPartyReviewApplicationV213({
  ...
  return {
    ...
    getReviewPage: async (disputeId) => {
      ...
      const reviewState = derivePartyReviewStateV213(stored.envelope, party);
      const currentAcknowledgment = currentDisclosureReviewAcknowledgmentV213(
        stored.envelope,
        party,
      );
      ...
      const disclosureReviewAvailable =
        ...
        !openChallenge;
      return {
        review_page_version: FIRST_PARTY_REVIEW_PAGE_VERSION_V213,
        review: reviewState,
        ...
        can_confirm: derivePartyConfirmationEligibilityV213(stored.envelope, party).eligible,
        ...
        disclosure_review_acknowledgment_statement: DISCLOSURE_REVIEW_ACKNOWLEDGMENT_STATEMENT_V213,
      };
    },
    acknowledgeDisclosureReview: async (disputeId) => {
      ...
      if (!disclosureReviewClosureCurrentV213(acknowledged.stored.envelope)) return acknowledged;
...
export function disclosureReviewAcknowledgmentCopyV213(): string {
  return DISCLOSURE_REVIEW_ACKNOWLEDGMENT_STATEMENT_V213;
}

export function reviewChallengeForPublicResponseV213(challenge: HumanHandoffChallengeV1) {
  return {
