...
import { validatePartyReviewStateV213, type PartyReviewStateV213 } from './party-review-state.js';
import {
...
import {
  ...
  type EnvelopeCeremonyCommandV213,
  type RecordPartyConfirmationOperationV213,
  type ReopenOwnFormationOperationV213,
} from './envelope-ceremony.js';
import {
  derivePartyConfirmationEligibilityV213,
  derivePartyReviewStateV213,
} from './party-review-state.js';

export const PARTY_REVIEW_PROTECTED_ACTION_VERSION_V213 =
  'juryai-party-review-protected-action-v1.2.0';

type ProtectedCeremonyOperationV213 =
  RecordPartyConfirmationOperationV213 | ReopenOwnFormationOperationV213;

export interface PartyReviewProtectedActionPayloadV213 {
  protected_action_version: typeof PARTY_REVIEW_PROTECTED_ACTION_VERSION_V213;
  review_state_hash: string;
  party_readback_hash: string;
  ceremony_command: EnvelopeCeremonyCommandV213 & {
    operation: ProtectedCeremonyOperationV213;
  };
}

export interface PreparePartyReviewChallengeInputV213 {
...
export type PreparePartyReviewChallengeResultV213 =
  | {
      status: 'prepared';
      party_id: PartyIdV213;
      review_state: PartyReviewStateV213;
      challenge: HumanHandoffChallengeV1;
      action_payload: PartyReviewProtectedActionPayloadV213;
    }
  | {
      status: 'rejected';
...
export type ExecutePartyReviewProtectedActionResultV213 =
  | {
      status: 'applied';
      party_id: PartyIdV213;
      envelope: CaseEnvelopeV213;
      ...
      prior_review_state: PartyReviewStateV213;
      resulting_review_state: PartyReviewStateV213;
    }
  | {
      status: 'rejected';
      reason_code:
...
function stateBinding(envelope: CaseEnvelopeV213, partyId: PartyIdV213) {
...
function operationFor(
  input: PreparePartyReviewChallengeInputV213,
): ProtectedCeremonyOperationV213 | null {
...
export function validatePartyReviewProtectedActionPayloadV213(
  value: unknown,
  ...
  if (
    ...
    payload.protected_action_version !== PARTY_REVIEW_PROTECTED_ACTION_VERSION_V213 ||
...
export function preparePartyReviewChallengeV213(
  input: PreparePartyReviewChallengeInputV213,
): PreparePartyReviewChallengeResultV213 {
  ...
  if (
    ...
    !derivePartyConfirmationEligibilityV213(input.envelope, partyId).eligible
  ) {
    return {
      status: 'rejected',
  ...
  const command = ceremonyCommandForV213(input.envelope, input.ids.command_id, operation);
  const dryRun = applyEnvelopeCeremonyCommandV213({
    envelope: input.envelope,
    command,
    execution_authority: partyAuthorityV213(input.envelope, partyId, 'first_party_human'),
  });
  ...
  const reviewState = derivePartyReviewStateV213(input.envelope, partyId);
  if (validatePartyReviewStateV213(reviewState).length > 0) {
    return {
      status: 'rejected',
      reason_code: 'invalid_transition',
  ...
  const actionPayload: PartyReviewProtectedActionPayloadV213 = {
    protected_action_version: PARTY_REVIEW_PROTECTED_ACTION_VERSION_V213,
    review_state_hash: reviewState.review_state_hash,
    party_readback_hash: reviewState.party_readback_hash,
    ceremony_command: command as PartyReviewProtectedActionPayloadV213['ceremony_command'],
  };
  const binding = stateBinding(input.envelope, partyId);
  if (
    !binding ||
    !validatePartyReviewProtectedActionPayloadV213(actionPayload, input.requested_action)
  ) {
    return {
      status: 'rejected',
      reason_code: 'invalid_transition',
...
export function executePartyReviewProtectedActionV213(input: {
  envelope: CaseEnvelopeV213;
  authenticated_subject_id: string;
  challenge: HumanHandoffChallengeV1;
  action_payload: PartyReviewProtectedActionPayloadV213;
...
}): ExecutePartyReviewProtectedActionResultV213 {
  const partyId = partyForSubject(input.envelope, input.authenticated_subject_id);
  ...
  if (!validatePartyReviewProtectedActionPayloadV213(input.action_payload, input.expected_action)) {
    return {
      status: 'rejected',
  ...
  const priorReviewState = derivePartyReviewStateV213(input.envelope, partyId);
  if (
  ...
  return {
    ...
    resulting_review_state: derivePartyReviewStateV213(applied.envelope, partyId),
  };
}

export interface PartyReviewPersistencePortV213 {
  getPartyReview(input: {
    dispute_id: string;
    authenticated_subject_id: string;
  }): Promise<PartyReviewStateV213 | null>;
  issuePartyReviewChallenge(input: {
    ...
    | { status: 'issued'; challenge: HumanHandoffChallengeV1; review_state: PartyReviewStateV213 }
  ...
  executePartyReviewAction(input: {
    ...
    | { status: 'applied'; review_state: PartyReviewStateV213 }
    | { status: 'rejected'; reason_code: string; message: string }
  >;
}

export interface PartyReviewApplicationV213 {
  getReview(caseId: string): Promise<PartyReviewStateV213 | null>;
...
export function createPartyReviewApplicationV213(input: {
  authenticated_subject_id: string;
  repository: PartyReviewPersistencePortV213;
