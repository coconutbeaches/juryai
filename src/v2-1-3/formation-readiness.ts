...
import {
  ...
  type FormationExplanatoryStateV213,
  type PartyIdV213,
...
import { currentPartyConfirmationV213 } from './party-projection.js';

export interface FormationReadinessV213 {
  readiness_version: typeof FORMATION_READINESS_VERSION_V213;
  ...
  required_current_confirmations: PartyIdV213[];
...
function authoritativeReadinessParts(envelope: CaseEnvelopeV213) {
  const openRequiredFields = Object.values(envelope.requirements)
    .filter(
      (requirement) =>
        requirement.required &&
        evaluateFormationRequirementV213(envelope, requirement).status !== 'satisfied',
    )
    .map((requirement) => requirement.requirement_id)
    .sort();
  const independentFormationPrerequisites = PARTY_IDS_V213.filter(
  ...
  const disclosureReviewPrerequisites = PARTY_IDS_V213.filter(
    (partyId) => currentDisclosureReviewAcknowledgmentV213(envelope, partyId) === null,
  ...
  const ineligibleRequiredEvidence = Object.values(envelope.evidence)
    ...
    .sort();
  const requiredCurrentConfirmations = PARTY_IDS_V213.filter(
    (partyId) => currentPartyConfirmationV213(envelope, partyId) === null,
  );
  const partyBindingBlockers = PARTY_IDS_V213.filter(
    (partyId) =>
...
export function authoritativeFormationExplanatoryStateV213(
  envelope: CaseEnvelopeV213,
): FormationExplanatoryStateV213 {
...
export function deriveFormationReadinessV213(envelope: CaseEnvelopeV213): FormationReadinessV213 {
  const parts = authoritativeReadinessParts(envelope);
  const expected = authoritativeFormationExplanatoryStateV213(envelope);
  ...
  return cloneCanonical({
    readiness_version: FORMATION_READINESS_VERSION_V213,
