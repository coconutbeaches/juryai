/** Private PostgreSQL persistence for V2.1.3 formation and review commands. */
...
import { assertValidCaseEnvelopeV213 } from './contract-validator.js';
import { applyEnvelopeCeremonyCommandV213, ceremonyCommandForV213 } from './envelope-ceremony.js';
import {
  type FormationCompilerRunAuditRecordV213,
  type FormationReplayRecordV213,
  type FormationReplayResponseV213,
  type FormationSourceAuditRecordV213,
  type FormationSubmissionAuditRecordV213,
...
import {
  ...
  FORMATION_PERSISTENCE_SCHEMA_V213,
  type ActiveFormationContextV213,
  type CommitCeremonyResultV213,
  type CommitControlledDisclosureInputV213,
  type CommitDisclosureReviewAcknowledgmentInputV213,
  type CommitExternalRelaySubmissionInputV213,
  type CommitExternalRelaySubmissionResultV213,
  type CommitFinalConfirmationInputV213,
  type FormationPartyPersistenceContextV213,
  type StoredFormationDisputeV213,
} from './formation-persistence.js';
import {
  executePartyReviewProtectedActionV213,
  preparePartyReviewChallengeV213,
  validatePartyReviewProtectedActionPayloadV213,
  type PartyReviewPersistencePortV213,
  type PartyReviewProtectedActionPayloadV213,
} from './party-review-application.js';
import { derivePartyReviewStateV213 } from './party-review-state.js';

const SCHEMA = FORMATION_PERSISTENCE_SCHEMA_V213;
...
export interface PostgresDisclosureReviewRepositoryOptionsV213 extends PoolConfig {
  pool?: Pool;
  clock?: { now: () => string };
  ids?: {
    next: (kind: PartyReviewIdentityKindV213, partyId: PartyIdV213) => string;
    public_reference: () => string;
  };
}

export type PartyReviewIdentityKindV213 =
  | 'challenge'
  | 'command'
  | 'confirmation'
  | 'confirmation_event'
...
function canonicalId(value: unknown, label: string): string {
  if (typeof value !== 'string' || !ID_PATTERN_V213.test(value)) {
    throw new TypeError(`${label} is not canonical.`);
  }
  return value;
}

function partyId(value: unknown): PartyIdV213 {
...
function hash(value: unknown, label: string): string {
  if (typeof value !== 'string' || !HASH_PATTERN_V213.test(value)) {
...
function decodeActionPayload(value: unknown): PartyReviewProtectedActionPayloadV213 {
  const payload = cloneCanonical(value as PartyReviewProtectedActionPayloadV213);
  if (!validatePartyReviewProtectedActionPayloadV213(payload)) {
    throw new TypeError('Stored protected action payload failed its canonical contract.');
  }
  return payload;
}

function defaultReviewIdentity(kind: PartyReviewIdentityKindV213, party: PartyIdV213): string {
...
function reviewActionIds(
  options: NonNullable<PostgresDisclosureReviewRepositoryOptionsV213['ids']>,
...
function sameCreationIdentity(requested: CaseEnvelopeV213, stored: CaseEnvelopeV213): boolean {
  const bindingIdentity = (envelope: CaseEnvelopeV213, party: PartyIdV213) => {
...
function decodeReplay(value: unknown): FormationReplayRecordV213 {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new TypeError('Replay record is invalid.');
  }
  const record = cloneCanonical(value as FormationReplayRecordV213);
  ...
  const idArray = (candidate: unknown): candidate is string[] =>
    ...
    candidate.every((id) => typeof id === 'string' && ID_PATTERN_V213.test(id));
  if (
    ...
    !HASH_PATTERN_V213.test(record.request_fingerprint) ||
    !Number.isSafeInteger(record.recorded_at_ms) ||
    Object.keys(responseRecord).sort().join(',') !== responseKeys.join(',') ||
    record.response.persistence_contract_version !== FORMATION_PERSISTENCE_CONTRACT_VERSION_V213 ||
    record.response.dispute_id !== record.dispute_id ||
    record.response.party_id !== record.party_id ||
    !ID_PATTERN_V213.test(record.response.submission_id) ||
    !ID_PATTERN_V213.test(record.response.source_turn_id) ||
    ...
    !HASH_PATTERN_V213.test(record.response.resulting_internal_envelope_hash) ||
    !Number.isSafeInteger(record.response.resulting_party_visible_version) ||
    record.response.resulting_party_visible_version < 1 ||
    !HASH_PATTERN_V213.test(record.response.resulting_party_projection_hash)
  ) {
    throw new TypeError('Replay record is invalid.');
  }
...
function decodeStored(row: StoredFormationRow): StoredFormationDisputeV213 {
  const envelope = cloneCanonical(row.envelope as CaseEnvelopeV213);
  ...
  if (
    ...
    throw new TypeError('Stored envelope identity disagrees with canonical V2.1.3 state.');
  }
  return {
    envelope,
...
function partyForSubject(envelope: CaseEnvelopeV213, subject: string): PartyIdV213 | null {
...
export class PostgresDisclosureReviewRepositoryV213 {
  readonly #pool: Pool;
  readonly #ownsPool: boolean;
  readonly #issuedContexts = new WeakSet<object>();
  readonly #clock: { now: () => string };
  readonly #ids: NonNullable<PostgresDisclosureReviewRepositoryOptionsV213['ids']>;

  constructor(options: PostgresDisclosureReviewRepositoryOptionsV213) {
    ...
    this.#ids =
      ...
      } satisfies NonNullable<PostgresDisclosureReviewRepositoryOptionsV213['ids']>);
    if (options.pool) {
              ...
              and exists (
                select 1 from pg_constraint
                 where conname = 'formation_disputes_contract_pair_v212'
                   and pg_get_constraintdef(oid) like '%juryai-case-envelope-v2.1.3%'
              ...
              and exists (
                select 1 from pg_constraint
                 where conname = 'formation_assurance_challenges_payload_binding'
                   ...
                   and pg_get_constraintdef(oid) like '%juryai-envelope-command-v2.1.3%'
              ) as ready`,
      [SCHEMA],
    );
    if (result.rows[0]?.ready !== true) {
      throw new Error('V2.1.3 disclosure-review persistence migration is incomplete.');
    }
  }
  ...
  async getPartyReview(input: {
  ...
  }): ReturnType<PartyReviewPersistencePortV213['getPartyReview']> {
    ...
    const result = await this.#pool.query<{ envelope: unknown }>(
      `select envelope from ${SCHEMA}.formation_disputes
        where dispute_id = $1 and schema_version = 'juryai-case-envelope-v2.1.3'
    ...
    return party ? derivePartyReviewStateV213(envelope, party) : null;
  }

  async issuePartyReviewChallenge(
    input: Parameters<PartyReviewPersistencePortV213['issuePartyReviewChallenge']>[0],
  ): ReturnType<PartyReviewPersistencePortV213['issuePartyReviewChallenge']> {
    const id = disputeId(input.dispute_id);
    const subject = canonicalId(input.authenticated_subject_id, 'authenticated_subject_id');
    return this.#transaction(async (client) => {
      const selected = await client.query<{ envelope: unknown }>(
        `select envelope from ${SCHEMA}.formation_disputes
          where dispute_id = $1 and schema_version = 'juryai-case-envelope-v2.1.3' for update`,
        [id],
      );
      ...
      const prepared = preparePartyReviewChallengeV213({
        envelope,
  ...
  async executePartyReviewAction(
    input: Parameters<PartyReviewPersistencePortV213['executePartyReviewAction']>[0],
  ): ReturnType<PartyReviewPersistencePortV213['executePartyReviewAction']> {
    const id = disputeId(input.dispute_id);
    const subject = canonicalId(input.authenticated_subject_id, 'authenticated_subject_id');
    ...
    return this.#transaction(async (client) => {
      ...
      const executed = executePartyReviewProtectedActionV213({
  ...
  async findById(idInput: string): Promise<StoredFormationDisputeV213 | null> {
  ...
  async listActiveContextsForPrincipal(
    subjectInput: string,
  ): Promise<ActiveFormationContextV213[]> {
    ...
    const result = await this.#pool.query(
      `select ${selectedFormationColumns('dispute')},
        ...
        where dispute.schema_version = 'juryai-case-envelope-v2.1.3'
  ...
  async resolvePartyContext(
    ...
    const result = await this.#pool.query(
      `select ${selectedFormationColumns('dispute')},
        ...
        where dispute.dispute_id = $1
          and dispute.schema_version = 'juryai-case-envelope-v2.1.3'
    ...
    const cursor = stored.envelope.control.party_views[party];
    const context: FormationPartyPersistenceContextV213 = Object.freeze({
      dispute_id: id,
      party_id: party,
      authenticated_subject_id: subject,
      internal_envelope_version: stored.internal_envelope_version,
  ...
  async createDispute(
    envelopeInput: CaseEnvelopeV213,
    ...
    return this.#transaction(async (client) => {
      ...
      if (!sameCreationIdentity(envelope, stored.envelope)) {
        throw new TypeError('A different V2.1.3 creation identity already uses this dispute id.');
      }
  ...
  async commitExternalRelaySubmission(
    ...
    registerCompilerVersion([], registry);
    ...
    if (
      ...
      registry.version.taxonomy_version !== 'juryai-p2-v0.3.0' ||
      ...
      throw new TypeError('Compiler artifact does not reproduce the committed V2.1.3 run.');
    }
    ...
    return this.#transaction(async (client) => {
      ...
      if (
        ...
        const rebased = rebaseExternalRelaySubmissionV213(submission, current.envelope);
      ...
      const applied = applyExternalRelaySubmissionV213({
        ...
        submission,
        execution_authority: partyAuthorityV213(current.envelope, party, 'external_relay'),
      });
      ...
      const sourceRecord: FormationSourceAuditRecordV213 = {
      ...
      const submissionRecord: FormationSubmissionAuditRecordV213 = {
        dispute_id: current.envelope.control.case_id,
        party_id: party,
        submission_id: submission.submission_id,
      ...
      const compilerRecord: FormationCompilerRunAuditRecordV213 = {
        persistence_contract_version: FORMATION_PERSISTENCE_CONTRACT_VERSION_V213,
      ...
      const replayRecord: FormationReplayRecordV213 = {
        dispute_id: current.envelope.control.case_id,
        party_id: party,
        client_turn_id: submission.source_turn.client_turn_id,
  ...
  async commitControlledDisclosure(
    ...
    return this.#transaction(async (client) => {
      ...
      const applied = applyEnvelopeCeremonyCommandV213({
        envelope: current.envelope,
        command: ceremonyCommandForV213(current.envelope, input.command_id, {
  ...
  async commitDisclosureReviewAcknowledgment(
    input: CommitDisclosureReviewAcknowledgmentInputV213,
  ): Promise<CommitCeremonyResultV213> {
    const id = disputeId(input.dispute_id);
    const subject = canonicalId(input.authenticated_subject_id, 'authenticated_subject_id');
    canonicalId(input.command_id, 'command_id');
    ...
    return this.#transaction(async (client) => {
      ...
      const command = ceremonyCommandForV213(current.envelope, input.command_id, {
      ...
      const applied = applyEnvelopeCeremonyCommandV213({
        envelope: current.envelope,
        command,
        execution_authority: partyAuthorityV213(current.envelope, partyId, 'first_party_human'),
      });
      if (applied.status === 'rejected') {
        return {
          status: 'domain_rejected',
  ...
  async commitFinalConfirmation(
    input: CommitFinalConfirmationInputV213,
  ): Promise<CommitCeremonyResultV213> {
    const id = disputeId(input.dispute_id);
    canonicalId(input.command_id, 'command_id');
    safeInteger(input.expected_internal_envelope_version, 'expected version', 1);
