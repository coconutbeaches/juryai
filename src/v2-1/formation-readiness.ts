...
import {
  ...
  type FormationExplanatoryStateV21,
  type PartyIdV21,
} from './case-envelope.js';
import { currentPartyConfirmationV21 } from './party-projection.js';

export interface FormationReadinessV21 {
  readiness_version: typeof FORMATION_READINESS_VERSION_V21;
  ...
  required_current_confirmations: PartyIdV21[];
...
function authoritativeReadinessParts(
  envelope: CaseEnvelopeV21,
): Omit<
  FormationReadinessV21,
  'blockers' | 'explanatory_consistency_issues' | 'readiness_version' | 'ready_for_bilateral_lock'
> & { authoritative_blockers: string[] } {
  const openRequiredFields = Object.values(envelope.requirements)
    .filter((requirement) => requirement.required && requirement.status === 'open')
    .map((requirement) => requirement.requirement_id)
    .sort();
  const independentFormationPrerequisites = PARTY_IDS_V21.filter(
    (partyId) => !envelope.parties[partyId].independent_formation_complete,
  ...
  const ineligibleRequiredEvidence = Object.values(envelope.evidence)
    ...
    .sort();
  const requiredCurrentConfirmations = PARTY_IDS_V21.filter(
    (partyId) => currentPartyConfirmationV21(envelope, partyId) === null,
  );
  const partyBindingBlockers = PARTY_IDS_V21.filter(
    (partyId) =>
...
export function authoritativeFormationExplanatoryStateV21(
  envelope: CaseEnvelopeV21,
): FormationExplanatoryStateV21 {
...
/** Stored blocker arrays are checked projections, never lock authority. */
export function deriveFormationReadinessV21(envelope: CaseEnvelopeV21): FormationReadinessV21 {
  const parts = authoritativeReadinessParts(envelope);
  const expected = authoritativeFormationExplanatoryStateV21(envelope);
  ...
  return cloneCanonical({
    readiness_version: FORMATION_READINESS_VERSION_V21,
