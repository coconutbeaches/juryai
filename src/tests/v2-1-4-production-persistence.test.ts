...
import { createPartyReviewApplicationV214 } from '../v2-1-4/party-review-application.js';
import {
  PostgresDisclosureReviewRepositoryV214,
  type PartyReviewIdentityKindV214,
} from '../v2-1-4/postgres-disclosure-review-repository.js';
import {
...
import type { FormationCompilerRunAuditRecordV214 } from '../v2-1-4/formation-persistence.js';

const DATABASE_URL = process.env.JURYAI_TEST_DATABASE_URL;
if (!DATABASE_URL) {
  throw new Error(
    'v2-1-4-production-persistence.test.ts requires JURYAI_TEST_DATABASE_URL pointing to an isolated database.',
  );
}

const pool = new Pool({ connectionString: DATABASE_URL });
const formation = new PostgresDisclosureReviewRepositoryV214({ pool });
const legacyFormation = new PostgresFormationRepositoryV211({ pool });
const invitations = new PostgresFormationInvitationRepositoryV214({
  pool,
...
function bindBothV214(envelope: CaseEnvelopeV214) {
  const subjectA = unique('subject_a');
  const subjectB = unique('subject_b');
  envelope = ceremonyV214(envelope, TRUSTED_SYSTEM_AUTHORITY_V214, {
    type: 'bind_party',
    party_slot: 'party_a',
...
function finalEnvelopeV214(label: string) {
  const bound = bindBothV214(createInitialCaseEnvelopeV214(unique(`dispute_${label}`)));
  let envelope = ceremonyV214(bound.envelope, TRUSTED_SYSTEM_AUTHORITY_V214, {
    type: 'open_controlled_disclosure',
  });
  for (const partyId of ['party_a', 'party_b'] as const) {
    envelope = ceremonyV214(envelope, partyAuthorityV214(envelope, partyId, 'first_party_human'), {
...
function idFactory() {
  return {
    next: (kind: PartyReviewIdentityKindV214, partyId: PartyIdV214) => {
...
function reviewStore() {
  return new PostgresDisclosureReviewRepositoryV214({
    pool,
    clock: { now },
    ids: idFactory(),
  });
...
function reviewApp(subject: string, store = reviewStore()) {
  return createPartyReviewApplicationV214({
...
describe('authorized V2.1.4 assurance migration', () => {
  it('preserves historical V2.1.2 row identity and rejects new schema cross-pairs', async () => {
    ...
    await pool.query(
      readFileSync(
        `${projectRoot}/supabase/migrations/20260904120000_v214_performance_formation_contract_pairs.sql`,
        'utf8',
      ),
    );
  ...
  it('keeps a historical V1/V2.1.1 row byte-identical while accepting the additive exact pair', async () => {
    ...
    const current = finalEnvelopeV214('new_assurance');
  ...
  it.each([
    ['juryai-party-review-protected-action-v1.0.0', 'juryai-envelope-command-v2.1.4'],
    ['juryai-party-review-protected-action-v1.1.0', 'juryai-envelope-command-v2.1.4'],
    ['juryai-party-review-protected-action-v1.3.0', 'juryai-envelope-command-v2.1.2'],
    ['juryai-party-review-protected-action-v1.3.0', 'juryai-envelope-command-v2.1.1'],
    ['juryai-party-review-protected-action-v9.9.9', 'juryai-envelope-command-v2.1.4'],
    ['juryai-party-review-protected-action-v1.3.0', 'juryai-envelope-command-v9.9.9'],
  ])(
    ...
    async (protectedVersion, commandVersion) => {
      const source = await pool.query<{
        ...
        where action_payload ->> 'protected_action_version' = 'juryai-party-review-protected-action-v1.3.0'
        limit 1`,
      );
  ...
  it('contains only the authorized constraint replacement and exact paired alternatives', () => {
    ...
    expect(migration).toContain('juryai-party-review-protected-action-v1.3.0');
    expect(migration).toContain('juryai-envelope-command-v2.1.4');
...
describe('V2.1.4 production PostgreSQL composition', () => {
  it('replays the current dispute after the same start request has already been mutated', async () => {
    ...
    const service = createProductionCaseServiceV214({
      authenticated_subject_id: subject,
    ...
    const audit = await pool.query<{ record: FormationCompilerRunAuditRecordV214 }>(
    ...
    expect(record.persistence_contract_version).toBe('juryai-v2.1.4-formation-persistence-v1');
  ...
  it('rejects a deterministic dispute ID collision from a different creator binding', async () => {
    const owner = createInitialProductionDisputeV214({
    ...
    ) as Parameters<typeof createInitialCaseEnvelopeV214>[1];
    let collision = createInitialCaseEnvelopeV214(owner.control.case_id, initialRequirements);
    collision = ceremonyV214(collision, TRUSTED_SYSTEM_AUTHORITY_V214, {
      type: 'bind_party',
      party_slot: 'party_a',
    ...
    await expect(formation.createDispute(collision)).rejects.toThrow(
      /different V2\.1\.4 creation identity/u,
    );
  });
  ...
  it('atomically creates, relays both independent formations, and opens disclosure exactly once', async () => {
    ...
    let envelope = createInitialCaseEnvelopeV214(unique('dispute_relay'), {
  ...
  it('binds an intended distinct Party B once and rejects possession, self-redemption, and replay generically', async () => {
    ...
    let envelope = createInitialCaseEnvelopeV214(unique('dispute_invite'));
    envelope = ceremonyV214(envelope, TRUSTED_SYSTEM_AUTHORITY_V214, {
      type: 'bind_party',
      party_slot: 'party_a',
      authenticated_subject_id: subjectA,
    ...
    const authority = productionInvitationAuthorityV214(true);
    ...
    ).toBe('juryai-case-envelope-v2.1.4');
    await expect(
  ...
  it('CAS-closes disclosure review exactly once and leaves substantive P3 absent', async () => {
    const seeded = bindBothV214(createInitialCaseEnvelopeV214(unique('dispute_final_cas')));
    let envelope = ceremonyV214(seeded.envelope, TRUSTED_SYSTEM_AUTHORITY_V214, {
      type: 'open_controlled_disclosure',
    });
    for (const partyId of ['party_a', 'party_b'] as const) {
      envelope = ceremonyV214(
        envelope,
        partyAuthorityV214(envelope, partyId, 'first_party_human'),
        {
          type: 'record_disclosure_review_acknowledgment',
          acknowledgment_id: unique(`disclosure_ack_${partyId}`),
          event_id: unique(`disclosure_ack_event_${partyId}`),
    ...
    const competing = new PostgresDisclosureReviewRepositoryV214({ pool });
  ...
  it('atomically assurance-binds exact V2.1.4 confirmation and reopen commands', async () => {
    const seeded = finalEnvelopeV214('confirm_reopen');
  ...
  it('rolls back envelope, challenge, receipt, and consumption together on durable identity collision', async () => {
    const collisionIds = {
      next: (kind: PartyReviewIdentityKindV214, partyId: PartyIdV214) => {
        ...
        const prefix: Record<
          Exclude<PartyReviewIdentityKindV214, 'receipt' | 'consumption'>,
          string
        > = {
          challenge: 'handoff_challenge',
          command: `command_${partyId}`,
    ...
    const collisionStore = new PostgresDisclosureReviewRepositoryV214({
      pool,
      clock: { now },
      ids: collisionIds,
    });
    const first = finalEnvelopeV214('rollback_first');
    const second = finalEnvelopeV214('rollback_second');
