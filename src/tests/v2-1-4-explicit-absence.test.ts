...
} from '../v2-1-4/explicit-absence-eval-corpus.js';
import { initialRequirementSet } from '../v2-1-4/initial-requirements.js';
...
import { evaluateFormationRequirementV214 } from '../v2-1-4/formation-requirements.js';
import { TRUSTED_SYSTEM_AUTHORITY_V214, type CaseEnvelopeV214 } from '../v2-1-4/case-envelope.js';
import {
...
import { deriveFormationReadinessV214 } from '../v2-1-4/formation-readiness.js';
import {
...
import { prepare, execute } from './v2-1-4-assurance-test-helpers.js';
...
describe('PR 7 explicit absence compiler replay corpus (not live model accuracy)', () => {
  ...
  it('old V2.1.1 and V2.1.2 validators do not acquire the new requirement type', () => {
    ...
    expect(validateCaseEnvelopeV214(createInitialProductionDisputeV214(start))).toEqual([]);
  });
...
async function canaryRegression(): Promise<CaseEnvelopeV214> {
  let envelope = createInitialProductionDisputeV214({
    ...start,
...
describe('PR 7 exact real-canary regression and full P2 readiness', () => {
  it('does not expose a P3 operation even to the trusted domain ceremony', () => {
    ...
    for (const type of ['adjudicate', 'render_verdict', 'execute_settlement', 'lock_case']) {
      const result = applyEnvelopeCeremonyCommandV214({
        ...
        command: {
          ...ceremonyCommandForV214(envelope, 'command_no_p3', {
  ...
  it('records explicit absence, preserves the target, and reaches bilateral HHC-3 readiness under V2.1.4', async () => {
    ...
    expect(renderPartyFormationReadbackV214(envelope, 'party_b').document).toContain(
      'explicit_absence',
    );
    expect(projectPartyFormationV214(envelope, 'party_a').opponent_material).toBeNull();
    envelope = ceremony(envelope, TRUSTED_SYSTEM_AUTHORITY_V214, {
      type: 'open_controlled_disclosure',
    });
  ...
  it('satisfaction uses exact requirement ID and cardinality, never global absence or a target date', async () => {
    ...
    expect(evaluateFormationRequirementV214(envelope, definition).status).toBe('unsatisfied');
