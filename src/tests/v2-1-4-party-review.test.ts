...
describe('V2.1.4 assurance-bound party review actions', () => {
  it('adds a distinct contract without reinterpreting the historical V1 payload', () => {
    ...
    expect(PARTY_REVIEW_PROTECTED_ACTION_VERSION_V214).toBe(
    ...
    expect(validatePartyReviewProtectedActionPayloadV214(prepared.action_payload)).toBe(true);
    ...
    expect(validatePartyReviewProtectedActionPayloadV214(relabeled)).toBe(false);
    expect(validatePartyReviewProtectedActionPayloadV1(relabeled)).toBe(false);
  });

  it('binds the exact V2.1.4 confirmation command through challenge and execution', () => {
  ...
  it('binds the exact V2.1.4 reopen command and invalidates the prior confirmation', () => {
    const envelope = finalConfirmationEnvelope();
  ...
  it('rejects any command mutation after the protected payload is authorized', () => {
    ...
    const result = executePartyReviewProtectedActionV214({
      envelope,
      authenticated_subject_id: envelope.parties.party_a.authenticated_subject_id!,
      challenge: prepared.challenge,
      action_payload: tampered as PartyReviewProtectedActionPayloadV214,
