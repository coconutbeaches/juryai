...
function finalConfirmationEnvelope(): CaseEnvelopeV212 {
  let envelope = createInitialCaseEnvelopeV212(unique('dispute_party_review'));
  ...
  for (const partyId of ['party_a', 'party_b'] as const) {
    envelope = ceremony(envelope, partyAuthorityV212(envelope, partyId, 'first_party_human'), {
  ...
  return ceremony(envelope, TRUSTED_SYSTEM_AUTHORITY_V212, {
...
function policy(action: PartyReviewProtectedActionV1) {
  const profile: IntentAssuranceProtocolProfileV1 = {
    ...
    profile_id: 'profile_v212_normal_hhc3',
...
function ids(partyId: PartyIdV212) {
  return {
...
function execute(
  ...
  return executePartyReviewProtectedActionV212({
    envelope,
    authenticated_subject_id: envelope.parties[partyId].authenticated_subject_id!,
...
describe('V2.1.2 assurance-bound party review actions', () => {
  it('adds a distinct contract without reinterpreting the historical V1 payload', () => {
    ...
    expect(validatePartyReviewProtectedActionPayloadV212(prepared.action_payload)).toBe(true);
    ...
    expect(validatePartyReviewProtectedActionPayloadV212(relabeled)).toBe(false);
    expect(validatePartyReviewProtectedActionPayloadV1(relabeled)).toBe(false);
  });

  it('binds the exact V2.1.2 confirmation command through challenge and execution', () => {
  ...
  it('binds the exact V2.1.2 reopen command and invalidates the prior confirmation', () => {
    const envelope = finalConfirmationEnvelope();
  ...
  it('rejects any command mutation after the protected payload is authorized', () => {
    ...
    const result = executePartyReviewProtectedActionV212({
      envelope,
      authenticated_subject_id: envelope.parties.party_a.authenticated_subject_id!,
      challenge: prepared.challenge,
      action_payload: tampered as PartyReviewProtectedActionPayloadV212,
