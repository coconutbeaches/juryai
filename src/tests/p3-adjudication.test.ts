...
class MockJuror implements P3ModelAdapter {
  ...
  async secondaryDecision(
    ...
    expect(anonymizedInitials.every((summary) => /^Juror \d+$/.test(summary.juror_id))).toBe(true);
    return {
      juror_id: this.jurorId,
  ...
  async peerScores(
    ...
    expect(anonymizedSecondaries.every((summary) => /^Juror \d+$/.test(summary.juror_id))).toBe(true);
