...
import { TRUSTED_SYSTEM_AUTHORITY_V21, type CaseEnvelopeV21 } from '../v2-1/case-envelope.js';
import {
...
import { PostgresFormationInvitationRepositoryV21 } from '../v2-1/postgres-formation-invitation-repository.js';
...
if (!DATABASE_URL) {
  throw new Error(
    'v2-1-invitation-persistence.test.ts requires JURYAI_TEST_DATABASE_URL pointing to an isolated database.',
  );
}
...
const formationStore = new PostgresFormationRepositoryV21({ connectionString: DATABASE_URL });
const invitationStoreA = new PostgresFormationInvitationRepositoryV21({
  connectionString: DATABASE_URL,
  account_commitment_secret: SECRET,
  invitation_ttl_ms: TTL_MS,
  clock: () => nowMs,
});
const invitationStoreB = new PostgresFormationInvitationRepositoryV21({
  connectionString: DATABASE_URL,
  account_commitment_secret: SECRET,
  invitation_ttl_ms: TTL_MS,
  clock: () => nowMs,
...
function bind(
  ...
  const result = applyEnvelopeCommandV21({
    envelope,
    command: commandForV21(envelope, unique(`command_bind_${party}`), {
      type: 'bind_party',
      party_slot: party,
      authenticated_subject_id: subject,
...
async function createUnboundDispute(label: string): Promise<{
  ...
  const envelope = bind(createInitialCaseEnvelopeV21(disputeId), 'party_a', partyASubject);
