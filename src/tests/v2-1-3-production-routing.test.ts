...
import { derivePartyReviewStateV213 } from '../v2-1-3/party-review-state.js';
...
import { decodeFirstPartyReviewV213 } from '../webmcp/browser/v2-1-3-review-contract.js';
...
function setup(enabled = true, activeOld: string[] = [], activeNew: string[] = []) {
  ...
  const routed = createProductionVersionedCaseServiceV213({
    enabled,
    legacy: legacy as never,
    v212: old as never,
    v213: current as never,
    resolveVersion: resolveVersion as never,
    startCaseId: (request) => (request === 'old_retry' ? 'dispute_old' : 'dispute_fresh'),
  });
  return { legacy, old, current, routed, resolveVersion };
}
const start = {
...
describe('PR 7 authoritative production routing and browser integration', () => {
  it('creates V2.1.3 for new starts and keeps the stable historical start identity', async () => {
    ...
    const next = createInitialProductionDisputeV213(start),
      prior = createInitialProductionDisputeV212(start);
    expect(next.control.case_id).toBe(prior.control.case_id);
    expect(next.control.schema_version).toBe('juryai-case-envelope-v2.1.3');
  ...
  it('requires explicit identity when V2.1.2 and V2.1.3 contexts coexist', async () => {
    expect(
  ...
  it('versions review/readback together and preserves historical decoding', () => {
    ...
    const current = page(
      derivePartyReviewStateV213(createInitialProductionDisputeV213(start), 'party_a'),
      'v2.1.3',
    );
    expect(decodeFormationReview(old)).toEqual(decodeFirstPartyReviewV212(old));
    expect(decodeFormationReview(current)).toEqual(decodeFirstPartyReviewV213(current));
    expect(() => decodeFirstPartyReviewV212(current)).toThrow();
    expect(() => decodeFirstPartyReviewV213(old)).toThrow();
  });
  it('routes first-party mutations and invitation redemption to persisted versions', async () => {
    ...
    const router = createVersionedFirstPartyService({
      ...
      resolveInvitationVersion: async (token) =>
        token === 'old-token' ? 'juryai-case-envelope-v2.1.2' : 'juryai-case-envelope-v2.1.3',
    });
  ...
  it('leaves the failed live canary out of mutation code and SQL', () => {
    ...
    for (const path of [
      ...
      'src/v2-1-3/production-case-service.ts',
      'src/v2-1-3/production-routing.ts',
      'supabase/migrations/20260904010302_v213_explicit_absence_contract_pairs.sql',
    ])
